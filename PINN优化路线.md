...
The runner reproduces 25.6 exactly on checked per-sample fields (`max_abs_diff=0`). On the 82 `rbc_representable_but_model_fail` rows, profile RMSE is `0.000509518351056 -> 0.000220386413188 m`, Er-like is `2.80015739379 -> 0.909941363416`, IoU/Dice is `0.32360140234/0.480524080842 -> 0.578523465369/0.709451842351`, and forward residual is `70.5944261489 -> 0.564105036956`. RBC-like control remains stable, while multi-pit/component-set rows are marked `not_suitable_for_rbc_refinement` and receive no RBC success credit.

The route boundary is now operational: forward refinement is a surface companion/post-hoc layer over `CURRENT_BASELINE.md` 20.85, not a transition. Multi-pit still needs a separate `component_set` branch. The unique next route is `A. surface refinement visualization/report package`, with any baseline transition still requiring a separate explicit request and review.
...
25.14 moved the surface RBC branch into a targeted top-up generation path, but it stopped at calibration instead of becoming a usable pack. The 120-row plan is deterministic and top-up-only: `80/20/20` split, `60/24/24/12` role coverage, fixed `sensor_z_m=0.008`, tri-axis `Bx/By/Bz`, three scan lines, and 201 x points. It is not an assembled training dataset, and it does not update `CURRENT_BASELINE.md`.

The real route boundary is the calibration failure. The `workers=1` calibration passed mesh pre-stage for all 24 rows, then COMSOL produced `22/24` successful forwards and two sample-level geometry failures. Because calibration is not zero-failure, the full 120 top-up generation is blocked. The only valid continuation is same-signature replacement for `balanced_interior|medium|narrow|sharp|interior` and `balanced_interior|deep|balanced|round|interior`, followed by rerunning calibration.

The `+120 training gate` remains a later independent stage. Only that later gate may explicitly assemble `v3_240 + topup_v1_120` and create an assembled manifest after full top-up success and validation.

## 2026-06-03 Stage 25.16 route sync: surface multi-pit label-v3b targets

25.16 changes the multi-pit route from label-schema audit back to a trainable target gate, but only after fixing the target semantics. The root cause from 25.15b was not missing COMSOL labels: v3 solved near-empty masks by making soft/valid support too broad, which turned component masks into union-like supports and caused merged collapse.

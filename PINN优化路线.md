...
25.15b 解释了 label v3 为什么在修掉 near-empty 症状后仍然失败。问题不是 raw labels 缺失，也不是 evaluator 把 merge 判错了；真正原因是 v3 把 component-local 的正样本和有效区域放得太宽，模型学成了 component identity 很弱的 union-like mask。

target audit 的关键证据很直接：soft OR/raw union ratio 均值为 `2.010499`，最大到 `2.451613`，全部样本（`112/112`）都有 union-like soft support。soft/valid duplicate fraction 最高到 `0.327496`，而且泄漏不只出现在 hard topology 子集：separated rows 也有非零 soft overlap，close rows 的 duplicate fraction 达到 `0.253333`。failure grouping 与这个机制一致：component_count=2、component_count=3、separated、close、touching、partially_overlapping rows 的 test merged rate 都是 `1.000000`。

route implication 是 `NEEDS_PINN_LABEL_DERIVATION_V3B`：修复仍保留在 `PINN_project` 内，但下一轮 label transform 必须在训练前收紧。v3b 应加入互斥 hard core，把 valid region 拆成 hard core / boundary halo / ignore overlap，让 soft halo 保持窄且不重叠；shared overlap pixels 在没有 ownership confidence 前只作为 ignore/diagnostic；depth 只在 hard core 加窄 owned boundary 上监督。`CURRENT_BASELINE.md` 保持不变。

## 2026-06-03 route sync: 25.15 surface multi-pit label-v3 training gate

25.15 测的是对 25.13 最干净的回答：保留 25.10 component-set model 和 loss mainline，只把脆弱的 hard target 换成 label-v3 soft/SDF/valid-region supervision。结果是一个有信息量的 `FAIL`，因为它证明 support sparsity 只是问题的一部分。

mask 不再塌到 near-empty：相对 25.13，component Dice 从 `0.005536` 升到 `0.034245`，union Dice 从 `0.002829` 升到 `0.061694`。但这个改善是 merged-mask 解法，不是 component separation。test merged rate 为 `1.000000`，depth RMSE 恶化到 `0.001106223 m`，recall 仍是 `0.674419`，所有必须单独报告的 hard slices 仍然 merge：component_count=3、partially_overlapping、touching_boundary 的 merged 都是 `1.000000`。

route implication 是停止 loss-stack tuning。唯一下一步是 `C. return to label-v3 derivation or generator/export schema; do not continue loss tuning`：检查 soft band/SDF targets 是否过宽，overlap/contact regions 是否需要显式 ownership confidence 或 anti-merge labels，以及 generator/export schema 是否需要 component-local separation support。`CURRENT_BASELINE.md` 保持不变。

## 2026-06-03 route sync: 25.14 surface multi-pit label-v3 derivation + validator

25.14 把 25.13b 的 schema diagnosis 落成了具体的 loader-side target design。关键结果是现有 raw component labels 足够支撑下一步：暂时不需要 COMSOL generator/export fix，也不需要修改 NPZ/data 就能测试下一条路线。

证据很直接：v2 hard ownership 仍作为 hard evaluation anchor，并把 duplicate ownership 保持在 `0`；v3 则围绕每个 raw component 增加 soft/local support。active components 为 `236`；v2 hard foreground support 均值 `99.851695` pixels，最小 `47`；v3 soft positive support 均值 `210.110169` pixels，最小 `126`。v3/v2 support ratio 均值为 `2.203184`，component_count=3 为 `2.445849`，partially-overlapping 为 `2.210708`，touching-boundary 为 `2.279145`。

route implication 是 `READY_FOR_25.15_TRAINING`：运行 label-v3 training gate，使用 25.10 loss mainline 加 `component_mask_target_v3_soft`、`component_sdf_target_v3`、`component_valid_region_mask` 和 `component_depth_target_v3`。不要复用 25.11/25.12 rebalance stack，不默认扩大模型，也不更新 `CURRENT_BASELINE.md`。

## 2026-06-03 route sync: 25.13b surface multi-pit generator/label schema audit after target-v2 collapse

25.13b 把 25.13 后混在一起的两个问题拆开了。ownership transform 作为 label-cleaning step 是有效的，但作为 training target 不够：它清掉了 duplicate pixels，却没有给模型更丰富的 component-local raster support。

证据排除了最简单的 generator-corruption 解释。v2 existing components 不是空的：active components 为 `236`，v2 foreground pixels 均值 `99.851695`，最小 `47`，只有 `3` 个 component 缩到 v1 面积的 `0.80` 以下。问题是这些 targets 在 `64x128` grid 中平均只占 `0.012188928`，而且仍是 hard binary masks。这解释了为什么 25.13 的 `merged=0.000000` 只是 near-empty mask collapse（`component Dice=0.005536`，`union Dice=0.002829`）。

route implication 是 `NEEDS_PINN_LABEL_DERIVATION_V3`：在任何新训练前，先在 `PINN_project` 内派生 soft/valid-region labels。label v3 应加入 soft component masks 或 SDF、component valid-region masks、overlap/contact masks，以及带 valid regions 的 component depth targets，同时继续保留 union OR/max 作为 evaluation。`CURRENT_BASELINE.md` 保持不变。

## 2026-06-03 route sync: 25.13 surface multi-pit component-set target-v2 training gate

25.13 测的是：把 25.12b ownership fix 接到稳定的 25.10 loss mainline 后是否足够。答案是不够：target v2 对 label cleaning 有用，但在当前 raster/depth supervision 下，单独作为 training target 不可行。

证据很直接：target v2 清掉了 label conflicts（`duplicate ownership 297 -> 0`，overlap-depth-conflict `271 -> 0`），同时保持 model architecture、`K=3`、split、Hungarian matching 和 `component_set_gate_v1` 不变。但学到的 masks 崩塌了：test component Dice 为 `0.005536`，union Dice 为 `0.002829`，recall 降到 `0.674419`，missed 升到 `0.325581`，extra 升到 `0.292683`。merged rate `0.000000` 不是成功，因为它来自 near-empty masks。

route implication 是停止 loss-stack training。唯一下一步是 `C. return to generator/label schema; do not continue loss tuning`：component-level supervision 很可能需要 generator/schema redesign 来提供更强的 positive raster/depth support，而不是只靠 ownership-resolved hard masks。`CURRENT_BASELINE.md` 保持不变。
...
真正的分界点是 component-local ownership。`25/112` 个样本存在 duplicated component foreground ownership，对应 `297` 个 duplicated component target pixels 和 `271` 个 overlap-depth-conflict pixels。问题集中在 partially-overlapping rows（`18/24`）和 three-component rows（`10/12`）。union supervision 会隐藏这类歧义，因为 OR/max 仍然正确；这也解释了为什么 25.11 能提高 union Dice 但会合并组件，以及为什么 25.12 的 loss penalties 没有恢复稳定 separation。

route implication 是 `READY_FOR_25.13_TRAINING`：把 `component_mask_target_v2`、`component_depth_target_v2` 和 `component_ownership_map` 作为 loader/training-time target transforms，回到 25.10 loss mainline，保留 union OR/max 作为 diagnostics，并单独报告 three-component rows。不要更新 `CURRENT_BASELINE.md`，也不要把它写成 baseline replacement。
...
代价很清楚。merged rate 相对 25.11 有改善（`0.900000 -> 0.700000`），但仍远差于 25.10（`0.200000`），而且伴随 recall collapse（`0.860465 -> 0.744186`）、missed-rate regression（`0.139535 -> 0.255814`）、extra-rate regression（`0.097561 -> 0.200000`）、union Dice 下降（`0.166233 -> 0.138075`），depth 也仍远高于 25.10（`0.000501023 m` 对 `0.000243315 m`）。three-component rows 仍然完全 merge。

route implication 是停止这条 training stack。唯一下一步是 `C. rollback to 25.10 loss mainline and redesign component raster/depth targets before further training`：审计 component-mask ownership、overlap/touching depth semantics，以及 labels 是否需要显式 component ownership/non-overlap constraints。`CURRENT_BASELINE.md` 保持不变。

## 2026-06-02 route sync: 25.11b surface multi-pit component-set merge-collapse audit

25.11b 钉住了第一轮 rebalance 为什么仍是 `PARTIAL`：objective 改善了 union-level agreement，却没有保住 component separation。所以 headline 看起来矛盾：union Dice 从 `0.130480` 升到 `0.166233`，recall 从 `0.837209` 升到 `0.860465`，extra/missed rates 也改善，但 merged rate 从 `0.200000` 跳到 `0.900000`。

这个 collapse 不只是 hard topology 子集问题。separated test rows 的 newly merged rate 为 `0.750000`，disconnected topology 的 newly merged rate 也是 `0.750000`，final validation mask/depth terms 仍占 weighted objective 的 `0.929449`。depth 也朝错误方向变化（`0.000243315 -> 0.000673627 m`），说明当前 foreground depth design 与同一个 merged-mask behavior 耦合。

route implication 现在很窄：下一步不要加 model capacity。唯一下一步是 `A. 25.12 component-separation-aware rebalance training`：限制或延迟 union mask loss，加入 component separation / anti-overlap penalties，加入 topology-aware merge penalties，并在 mask separation 之后 staged component-normalized depth。`CURRENT_BASELINE.md` 保持不变。

## 2026-06-02 route sync: 25.11 surface multi-pit component-set mask/depth loss rebalance training

25.11 测的是对 25.10b diagnosis 的直接回答：在改变 representation 或 model capacity 前，先 rebalance objective。路线仍限制在现有轻量 `C1_fixed_K_component_set` architecture、固定 `K=3`、固定 split `72/20/20`，以及同一个 min-over-slot-permutations Hungarian matching 内。

结果有用但不够：`mask_depth_rebalance_v1` 改善了 union-level shape signal 和 component detection，但没有解决 component separation。test recall 从 `0.837209` 升到 `0.860465`，missed rate 从 `0.162791` 降到 `0.139535`，extra rate 从 `0.142857` 降到 `0.097561`，union mask Dice 从 `0.130480` 升到 `0.166233`。同时 component mask Dice 基本持平或略差（`0.109562 -> 0.108737`），depth-grid RMSE 恶化（`0.000243315 -> 0.000673627 m`），merged rate 跳到 `0.900000`。

route implication 是下一步问题不是 generic capacity，而是 union-mask supervision、component separation、depth normalization/staging 与 touching/overlap topology 之间的相互作用。唯一下一步是 `B. 25.11b targeted rebalance or topology-focused failure audit`；`CURRENT_BASELINE.md` 保持不变，multi-pit 仍不获得 six-parameter RBC success credit。

## 2026-06-02 route sync: 25.10b surface multi-pit component-set failure audit

25.10b 在不改变 baseline 边界的前提下解释了 25.10 的 `PARTIAL` 结果。component-set gate 学到了 existence 和 coarse geometry（`test recall=0.837209`，`center_error_mean=0.004284 m`，`L/W/D relative error=0.154552`），但没有学到可用的 raster masks（`component Dice=0.109562`，`union Dice=0.130480`），depth-grid RMSE 也只是略微优于 degenerate baselines。

audit 把 implementation bugs 和 route problems 分开了。target masks 与 union masks 内部对齐（`target_union_mask_iou_mean=1.000000`），center labels 与 mask centroids 一致（mean error `0.000059604 m`），empty component slots 的 mask/depth target 为零。slot matching 结构上也正确：trainer 使用 min-over-slot-permutations，并把 non-existence slots 从 parameter、shape、mask、depth losses 中屏蔽。

route implication 很直接：先修 learning objective，而不是先改 representation。唯一下一步是 `B. enter 25.11 mask/depth loss rebalance training`，同时把 three-component scarcity 和 touching/overlap topology 作为必须跟踪的 audit slices。multi-pit 仍在 six-parameter RBC success credit 之外，`CURRENT_BASELINE.md` 保持不变。

## 2026-06-02 route sync: 25.10 surface multi-pit component-set training gate

25.10 回答了第一个 component-set learning-signal 问题：`C1_fixed_K_component_set` 路线不是死路，但还不是稳定 model route。轻量 gate 在 `comsol_surface_multipit_component_set_pilot_v1` 上训练后，比 empty 或 one-slot degenerate predictors 更能学到 component existence 和 coarse geometry；test recall 为 `0.837209`，missed rate `0.162791`，extra rate `0.142857`，mean predicted component count 为 `2.1`。

限制机制在 raster/topology 侧。component mask Dice 仍是 `0.109562`，union Dice `0.130480`，validation union Dice 只有 `0.071413`，depth-grid RMSE 也只是略优于 degenerate baselines。最硬的证据来自 three-component test subset：recall 降到 `0.666667`，merged rate 为 `1.0`。所以 gate decision 是 `PARTIAL`，不是 PASS。

下一步是 `B. run 25.10b failure audit`：在任何 25.11 stronger training protocol 前，先检查 merged/missed rows、overlap/touching subsets、slot permutation 和 three-component samples。这仍与 surface RBC baseline 分离；frozen 20.85 和 surface forward-refinement runner 只作为 comparators，`CURRENT_BASELINE.md` 保持不变。

## 2026-06-02 route sync: 25.9b surface multi-pit component-set top-up pack

25.9b 把 multi-pit component-set plan 转成了 validated dataset candidate。生成的 top-up pack 是 `comsol_surface_multipit_topup_pack_v1`：COMSOL rows 成功 `96/96`，split 为 `63/17/16`，component counts 为 `2=84` 和 `3=12`，separated/close/touching/partially-overlapping cases 覆盖均衡，并且有 explicit component masks/depth grids。

assembled training-gate candidate 是 `comsol_surface_multipit_component_set_pilot_v1`，由旧 25.2 multi-pit seed `16` 加 top-up `96` 组成，`N=112`，split `72/20/20`，固定 `K=3`，component labels 完整。registry 和 manifest 明确边界：`train_ready_candidate=true`，`baseline_ready=false`，不做 latest/newest discovery，不做 `CURRENT_BASELINE.md` transition，也不给 six-parameter RBC success credit。

唯一下一步是 `A. enter 25.10 component-set training gate`。该 gate 应使用 Hungarian matching 和 permutation-invariant losses 训练/评估 component-set branch；frozen 20.85 和 surface forward-refinement runner 对 multi-pit 只是 negative/control comparators，不是 replacement baselines。

## 2026-06-02 route sync: 25.9 surface multi-pit component-set branch plan

25.9 把 25.8 route decision 落成了 multi-pit representation branch 的具体计划。核心边界是：`multi_pit_two_component_surface_defect` 不是 six-parameter RBC failure case，而是 component-set representation problem。frozen 20.85 和 25.7/25.8 forward-refinement runner 仍可作为 comparators，但不获得 multi-pit success credit。

当前 16 条 multi-pit rows 足够 audit labels，但不足以训练。它们全是 `component_count=2`，在 `component_params_json` 中有 component centers 和 L/W/depth，并带有 union `projected_mask_2d` 与 `depth_grid_m`。下一包缺的 schema pieces 是 per-component rotation、component-level masks/depth grids，以及 explicit separation/touching/overlap topology labels。首选 representation route 是 `C1 fixed_K_component_set`，使用 `K=3`、Hungarian matching、component existence/geometry losses 和 union mask/depth losses。

下一步是 `A. execute multi-pit COMSOL top-up generation`：target top-up `N=96`，assembled `N=112`，split `72/20/20`，使用 two/three-component Boolean subtract 并导出 Bx/By/Bz，同时记录 Boolean、mesh、solver、component-count、merged-domain 和 label/union mismatch 的 failure ledger。`CURRENT_BASELINE.md` 保持不变，任何训练或 baseline transition 都必须留到后续显式 gate。

## 2026-06-02 route sync: 25.8 surface forward-refinement report package

25.8 把已验证的 25.7 runner 转成 report 和 visualization package。关键 route judgment 不变：frozen 20.85 仍是当前 surface RBC baseline，forward-refinement runner 是 companion/post-hoc repair layer，RBC oracle 是 evaluation ceiling，不是 runtime model 或 baseline replacement。

report 确认 companion runner 对 RBC representation 有效但 frozen inverse 失败的 rows 有用。在 `82` 条 `rbc_representable_but_model_fail` rows 上，profile RMSE 为 `0.000509518351056 -> 0.000220386413188 m`，Er-like 为 `2.80015739379 -> 0.909941363416`，IoU/Dice 为 `0.32360140234/0.480524080842 -> 0.578523465369/0.709451842351`，forward residual 为 `70.5944261489 -> 0.564105036956`。RBC-like control 保持稳定，同时 audit 用 `17` 条 degraded target rows 保留了 local non-improvement 的可见性。

剩余 surface 问题现在更清楚：multi-pit / component-set rows 不适合 six-parameter RBC refinement，也不给 RBC success credit。唯一下一步是 `A. component-set branch for multi-pit`；`CURRENT_BASELINE.md` 保持不变，任何 baseline transition 仍需要单独明确请求和 review。

## 2026-06-02 route sync: 25.7 surface forward-refinement inference runner

25.7 把 25.6 的 F0/R1 结果锁成具体 companion runner，而不是新 baseline。runtime chain 是 frozen 20.85 baseline prediction、observed `delta_b` feature extraction、导出的 `ridge_param_only_linear_alpha_10` artifact，以及对 `L_m/W_m/D_m/wLD/wWD/wLW` 的 post-hoc refinement。artifact body 保持 ignored 在 `checkpoints/` 下；提交的 manifest 只记录 identity、sha256、protocol、feature columns、bounds 和 allowed/forbidden use。

runner 在 checked per-sample fields 上精确复现 25.6（`max_abs_diff=0`）。在 `82` 条 `rbc_representable_but_model_fail` rows 上，profile RMSE 为 `0.000509518351056 -> 0.000220386413188 m`，Er-like 为 `2.80015739379 -> 0.909941363416`，IoU/Dice 为 `0.32360140234/0.480524080842 -> 0.578523465369/0.709451842351`，forward residual 为 `70.5944261489 -> 0.564105036956`。RBC-like control 保持稳定，multi-pit/component-set rows 被标记为 `not_suitable_for_rbc_refinement`，不获得 RBC success credit。

路线边界已经可执行：forward refinement 是覆盖在 `CURRENT_BASELINE.md` 20.85 上的 surface companion/post-hoc layer，不是 transition。multi-pit 仍需要单独的 `component_set` branch。唯一下一步是 `A. surface refinement visualization/report package`，任何 baseline transition 仍需要单独明确请求和 review。

## 2026-06-02 route sync: 25.6 surface forward-refinement formal benchmark

25.6 把 25.5 的 F0/R1 candidate 固化成 formal benchmark，而不是 baseline replacement。它重放同一个 `ridge_param_only_linear_alpha_10`、`alpha=10.0`、`lambda_param=1.0`、frozen 20.85 six-parameter initialization，以及 `L_m/W_m/D_m/wLD/wWD/wLW` post-hoc optimization；没有搜索 surrogate family、alpha、loss weight 或 optimizer。

candidate 的真实适用范围很窄但有价值：`rbc_representable_but_model_fail`。在这 `82` 行上，baseline/refined/oracle profile RMSE 是 `0.000509518351056 / 0.000220386413188 / 0.0000784896954944 m`，Er-like 是 `2.80015739379 / 0.909941363416 / 0.28925522333`，IoU/Dice 从 `0.32360140234/0.480524080842` 改善到 `0.578523465369/0.709451842351`，forward residual 从 `70.5944261489` 降到 `0.564105036956`。formal gates 全部 `10/10` PASS，RBC-like control 未退化。

路线边界保持不变：这是 refinement candidate，不是新的 current baseline。multi-pit 仍是 `component_set` representation branch，不获得 RBC success credit。唯一下一步是对 25.5 F0/R1 candidate 做 no-baseline-transition formal benchmark/audit，`CURRENT_BASELINE.md` 保持不变。

## 2026-06-02 route sync: 25.5 surface feature-space forward-consistency refinement diagnostic

25.5 第一次执行 surface forward-consistency refinement diagnostic。选定技术路线是 `F0_feature_space_consistency + R1_low_dim_param_refinement`：F0 对比 generated RBC profile 的 predicted compact delta_b-derived features 与 observed delta_b/BxByBz features；R1 从 frozen 20.85 的 `L_m/W_m/D_m/wLD/wWD/wLW` 开始，不改 model weights，只做 parameter-bound/profile-regularity 约束下的参数修正。

预期目标子集上的 route signal 很强：profile RMSE `0.000509518351056 -> 0.000220386413188 m`，Er-like `2.80015739379 -> 0.909941363416`，IoU/Dice `0.32360140234/0.480524080842 -> 0.578523465369/0.709451842351`，forward residual `70.5944261489 -> 0.564105036956`。RBC-like control 也改善，全部 `10/10` acceptance gates 通过。

边界不变：这是 diagnostic refinement stage，不是训练，也不是 baseline transition。multi-pit 因为是 `component_set` representation failure，继续排除在 RBC success accounting 之外；它需要后续 component-set decoder branch。

## 2026-06-02 route sync: 25.4 surface forward-consistency refinement plan

25.4 是 plan-only 阶段，把 25.3 的 audit 输出转成 forward-consistency refinement 方案。目标集固定为 `82` 条 `rbc_representable_but_model_fail` rows，`22` 条 `rbc_representable_and_model_pass` rows 作为 reference，`16` 条 `multi_pit_two_component_surface_defect` / `rbc_not_representable` rows 作为 excluded negative controls。

选定第一条路线是 `F0_feature_space_consistency + R1_low_dim_param_refinement`。F0 使用 observed `delta_b` 的 feature-space consistency，R1 从 frozen 20.85 的六参数预测初始化，只优化 `L_m/W_m/D_m/wLD/wWD/wLW`，不改变 model weights。`F1_neural_forward_surrogate`、`F2_cached_COMSOL_local_refinement` 和 `F3_direct_COMSOL_refinement` 只保留为后续或不推荐选项。

验收边界很明确：同一 target subset 上 profile RMSE、Er-like、IoU/Dice、area error、RBC-like control 和 forward residual 必须协同改善。multi-pit 仍从 RBC success accounting 中排除，因为它是 `component_set` representation failure。

## 2026-06-02 route sync: 20.99 internal / buried defect feasibility schema

20.99 把 internal / buried defect 从 surface / near-surface RBC baseline 中拆出来。内部/埋藏缺陷不能直接使用当前 surface RBC schema，因为它需要 `burial_depth_m` / `depth_to_surface_m`、`defect_center_xyz_m`、`shape_type`、内部空腔尺寸或体素 mask，以及独立 ground truth method。

第一种 internal representation 应是 `shape_type + L/W/D + burial_depth + center_xyz`。`internal_ellipsoid_params` 和 `internal_cuboid_params` 是第一批 parametric extensions；3D occupancy / cavity mask 是后续目标；surface-equivalent projected profile 只作为 QA/comparator output。Bx/By/Bz 仍是主线输入，Bz-only 只能作为低能力 diagnostic branch，不能替代 true 3D internal mainline。

选定路线是 `A. execute internal COMSOL smoke pack after metadata confirmation`：后续 6-12 sample smoke pack，当前计划为 12 行，覆盖 ellipsoid/cuboid/sphere-like 与 shallow/medium/deep burial。本阶段没有运行 COMSOL、没有训练、没有生成 data/NPZ，也没有更新 `CURRENT_BASELINE.md`。

## 2026-06-02 route sync: 25.3 current baseline generalization audit

25.3 在 `comsol_surface_shape_extension_pilot_v1` 上审计 frozen 20.85/20.86 surface RBC baseline。结论很明确：frozen 20.85 baseline 不能泛化到这个 pilot distribution。它只通过 `22/120`，non-RBC pass 为 `19/96`，即使 RBC-like smooth pit control 也只有 `3/24`。

这意味着下一步不能直接做 20.85-style baseline transition，也不能假设 non-RBC 失败主要是 label/schema 问题。RBC oracle 可表示 `104/120`，non-RBC 可表示 `80/96`，所以很多 single-component non-RBC failures 是 frozen inverse/generalization failure；例外是 multi-pit，`16/16` 都是 `rbc_not_representable`，属于 component-set branch。

选定路线是 `D. forward-consistency refinement plan`：先定义 no-training plan，用 forward residual 和 RBC-like control stability 审计这个 pilot，同时把 `component_set` 保留为独立 multi-pit branch。`CURRENT_BASELINE.md` 仍是 20.85/20.86 surface RBC profile-depth baseline。

## 2026-06-02 route sync: 25.2 surface shape-extension COMSOL pilot pack

25.2 生成并验证了 `comsol_surface_shape_extension_pilot_v1`。pilot 为 `N=120`，split `72/24/24`，`rbc_like_smooth_pit=24`，六类 non-RBC-like 各 `16`。Boolean subtract、mesh precheck、solve、finite `Bx/By/Bz`、`delta_b=b_defect-b_no_defect`、profile/depth labels、projected masks、topology labels 和 explicit `representation_target` 都通过验证。

关键边界是 representation。RBC-like control rows 保持 `representation_target=six_param_rbc`；non-RBC-like rows 使用 `profile_basis`、`depth_grid`、`component_set` 或 `polygon_or_contour`。validation 确认 finite `Bx/By/Bz`、`delta_b=b_defect-b_no_defect`、Boolean/mesh/solve pass、topology coverage、component labels、projected masks，以及没有把 non-RBC shapes 塌回旧 six RBC parameters。

路线进入 25.3：用 frozen 20.85/20.86 surface RBC baseline 审计该 pilot pack，并量化 non-RBC failure modes。训练仍不在范围内，直到后续明确的 25.4 gate；`CURRENT_BASELINE.md` 保持不变。

## 2026-06-01 route sync: 25.1 surface shape-extension dataset plan

25.1 把 surface branch 从 Piao-NLS diagnostic/QC 收口后，切到 non-RBC-like surface defect coverage。该阶段定义 taxonomy、label schema、split coverage、COMSOL feasibility、model routes 和 acceptance gates，不运行 COMSOL、不训练、不生成 data/NPZ，也不更新 `CURRENT_BASELINE.md`。

核心边界是 representation。RBC-like control rows 继续使用 `six_param_rbc`；non-RBC-like rows 使用 `profile_basis`、`depth_grid`、`component_set` 或 `polygon_or_contour`。multi-pit 需要 component-level labels，crack-like 需要 aspect/rotation，irregular corrosion 保留 depth-grid/profile targets，forward-consistency 是后续 gate。

路线进入 25.2：在 review 通过后执行 surface shape-extension COMSOL pilot generation。目标 pilot `N=120`，split `72/24/24`，RBC-like control `24`，六类 non-RBC-like 各 `16`；这不是训练，也不是 baseline transition。
## 2026-06-01 route sync: 24.2 surface RBC NLS-lite feature fusion

24.2 把 surface RBC NLS-lite 路线从 feature-only comparison 推进到 neural feature fusion。固定输入契约是 `delta_b/BxByBz + nlslite_*`；`sample_id` 只用于 join/reporting，`split` 只用于 train/val/test routing，所有 feature/target scalers 都是 train-only。本阶段没有 COMSOL、没有修改 data/NPZ、没有提交 checkpoint，也没有更新 `CURRENT_BASELINE.md`。

选定路线是 `F1_late_fusion`，seed `123` 来自 multi-seed validation-only selection。test metrics 为 total normalized MAE `0.598309`，L/W/D MAE `1.816667/1.657295/0.654960 mm`，wMAE `0.183249`，profile RMSE `0.000317238 m`，Er-like `0.267248`，IoU/Dice `0.793564/0.877942`。它相对 20.85/20.77 和 24.1 在 total、wMAE、profile RMSE 与 Dice 上都有改善，因此形成 surface feature-fusion candidate。

路线边界保持明确：24.0A 是 three-line NLS-lite feature source，24.0B 是未来 richer y-line ROI matrices 的 NLS-full-compatible interface，24.1 是 feature-only comparator，24.2 是需要 formal rerun 后才能讨论 baseline 的 candidate。在单独请求并 review baseline transition 前，`CURRENT_BASELINE.md` 仍保持 20.85 surface RBC profile-depth baseline。
...
当前路线状态：
- `wLD / wWD / wLW` 仍是 auxiliary diagnostics，不是 headline pass/fail metrics。
- `profile_depth_rmse_m` / Er-like profile reconstruction 才是 true 3D RBC-style branch 的主指标。
- 20.77 仍是更强的 profile reconstruction reference。
- 20.81 仍可作为 visual/mask comparator。
- 20.83 是 negative result，不能写成 baseline 或 `CURRENT_BASELINE` replacement。

下一步路线不应继续在同一个 six-param loss 上做窄幅 weight tweak。如果继续该路线，应优先做 profile-native output representation experiment，同时保留 `exact_piao_rbc=False` 和 `rbc_style_approximation=True`。

# 2026-05-26 Stage 20.84 route consolidation

20.84 在没有新训练、没有生成数据的情况下，收口了当前 true 3D RBC candidate 的歧义。该分支现在应把 20.77 作为 profile/depth benchmark candidate，把 20.81 作为 non-negative projected-mask / visual comparator，把 20.83 作为当前 R1 profile-primary loss design 的 negative evidence。

关键路线修正是：projected mask Dice 与 3D profile quality 必须分开。20.83 在三个候选中 Dice 数值最高，但 profile RMSE 差于 20.77，因此不是 replacement。20.81 保留为 visual comparator，因为它不是 negative profile-depth gate，且 projected-mask behavior 较强。

下一步路线：如果要 consolidation，则用 20.77 做 formal benchmark rerun；如果要改进方法，则转向更 profile-native 的 output representation。不要更新 `CURRENT_BASELINE.md`，不要把 v3_240 写成 baseline，并继续保持 `exact_piao_rbc=False` / `rbc_style_approximation=True`。

# 2026-05-27 Stage 20.90 route note

20.90 增加了第一个围绕当前 true 3D RBC baseline 的 physical acquisition variation 小型 COMSOL diagnostic。diagnostic 使用 12 个 base geometries、96 条 COMSOL variant rows，以及 36 条 nominal-derived postprocess axis-misalignment rows。它不是训练，不是 baseline replacement，也不改变 `CURRENT_BASELINE.md`。

路线含义很窄但重要：liftoff 是当前主要 robustness blocker。raw source/amplitude variation 可以被固定的 20.89 `per_axis_rms_train_stats` diagnostic calibration 缓解，但 calibration 只是 diagnostic/acquisition caveat，因为它会改变 clean-scale behavior，不能升级进 baseline。scan-line offset 和 small sensor_x axis misalignment 在这个 pack 中风险较低。

下一步路线方向：在 internal defect feasibility 前，先构建 dedicated COMSOL liftoff robustness / augmentation data design。internal/buried defects 仍需要独立 label schema，不应混入当前 surface RBC profile-depth baseline。

# 2026-05-27 Stage 20.91 route note

20.91 是 20.90 liftoff diagnostic 的 plan-only follow-up。它定义 dedicated liftoff augmentation pack：48 个 base geometries，每个 base 有四个 paired liftoff levels（`0.006 / 0.008 / 0.010 / 0.012 m`），计划 192 条 COMSOL rows。它不运行 COMSOL、不生成 data、不训练、不更新 `CURRENT_BASELINE.md`。

路线含义是：liftoff robustness 应先于 internal/buried defects 和 real-data claims 处理。calibration 仍只是 acquisition diagnostic caveat。pack 生成并验证后，下一 method gate 应比较当前 unconditioned baseline family 与 scalar `sensor_z_m` conditioned liftoff-aware variant。

# 2026-05-27 Stage 20.91b route note

20.91b 执行 dedicated liftoff COMSOL pack，并生成 full paired dataset：48 个 base geometries，每个 base 四个 liftoff levels（`0.006 / 0.008 / 0.010 / 0.012 m`），`192/192` successful rows，`48/48` complete paired bases。该 pack 以 `comsol_true_3d_rbc_liftoff_aug_pack_v1` 注册到 `true_3d_piao_style_liftoff_robustness` 下。

这不改变 `CURRENT_BASELINE.md`。路线进入 20.92 liftoff-aware training gate：比较当前 unconditioned model family 与 scalar `sensor_z_m` conditioned variant。internal/buried defects 继续 deferred，直到 surface-defect liftoff robustness 被理解清楚。

# 2026-05-27 Stage 20.92 route note

20.92 测试 full 20.91b liftoff pack 是否能产生 liftoff-robust true 3D RBC model。data gate 通过：`comsol_true_3d_rbc_liftoff_aug_pack_v1` 具备 48 个 paired base geometries、每个 base 四个 liftoff levels，以及 grouped base-level train/val/test splits，没有 geometry leakage。

路线结果是 mixed。liftoff augmentation 明显帮助 non-nominal rows：selected `C1_unconditioned_liftoff_aug` seed `123` 把 test non-nominal profile RMSE 从 fixed C0 baseline 的 `0.000874310 m` 降到 `0.000659761 m`，projected Dice 从 `0.683351` 升到 `0.833129`。但它没有保护 nominal operating point：nominal `0.008 m` profile RMSE 从 `0.000333059 m` 退化到 `0.000809011 m`。C2 sensor_z conditioning 没有通过 validation selection 胜出，它与 C1/C2 的比较只保留为 post-hoc diagnostic。

因此 20.92 不是 baseline transition，也还不是 formal robustness candidate。`CURRENT_BASELINE.md` 仍是 20.85 nominal true 3D RBC profile-depth baseline。下一步应审计 liftoff failure cases，并在 internal defects、real-data claims 或更大 COMSOL expansion 前设计 nominal-preserving liftoff objective 或 paired-liftoff consistency protocol。

# 2026-05-27 Stage 20.93 route note

20.93 用 route correction 收口第一轮 liftoff-aware training gate。20.92 的有用信号真实但不完整：unconditioned liftoff augmentation 改善了 non-nominal rows，却严重破坏了 nominal `0.008 m` behavior。失败机制不是数据量不够，而是 unconditioned mixed-liftoff inverse problem 加上 validation 没有显式保护 20.85 nominal baseline path。

下一步不应继续 C1-style unconditional augmentation。主实验应改为 nominal-preserving baseline+liftoff adapter：锚定 20.85 nominal path，并学习一个小的 `sensor_z_m`-conditioned correction 处理 non-nominal liftoff。revised full `sensor_z_m`-conditioned model 保留为 secondary ablation；如果 adapter objective 稳定，可以加入 paired liftoff consistency 作为 regularizer。

该 training gate 前不需要新 COMSOL。`CURRENT_BASELINE.md` 保持不变，internal/buried defects 和 real-data alignment 继续 deferred，直到 liftoff robustness 能同时保护 nominal behavior 并改善 non-nominal rows。

# 2026-05-27 Stage 20.94 route note

20.94 验证了 20.93 的 route correction。选中的 `A2_latent_residual_adapter` 使用 frozen 20.85/20.77 baseline latent、baseline six-parameter prediction 和 `sensor_z_m` 来预测 residual correction。它基本保持 nominal operating point，同时把 non-nominal liftoff profile RMSE 约减半。

这形成 liftoff robustness candidate，但不是 baseline replacement。当前 baseline 仍是 20.85 nominal true 3D RBC profile-depth baseline。下一步应对 A2 做 formal liftoff benchmark，继续保留 validation-only selection 和 grouped base splits。internal/buried defects 与 real-data alignment 继续 deferred，直到 liftoff benchmark 确认稳定性。

# 2026-05-28 Stage 20.95 route note

20.95 把 20.94 A2 结果 formalize 为 companion robustness path。`A2_latent_residual_adapter` seed `2026` 保持 nominal `0.008 m` operating point（`0.000333059 m -> 0.000335821 m`，`+0.829%`），并改善 non-nominal liftoff profile RMSE（`0.000874310 m -> 0.000437214 m`，`-49.993%`），non-nominal Dice 从 `0.683351` 升到 `0.842378`。

路线状态被刻意拆开：`CURRENT_BASELINE.md` 仍是 20.85 nominal true 3D RBC profile-depth baseline，A2 只接受为它的 liftoff companion module。`sensor_z_m` 是 multi-liftoff 或 real-experimental inference 的必需 metadata field。internal/buried defect feasibility 继续 deferred；下一步是 liftoff-conditioned inference smoke，验证 baseline + A2 loading 和 `sensor_z_m` metadata contract。

# 2026-05-28 Stage 20.96a route note

20.96a 解决了 20.96 blocker：A2 现在有可加载 inference artifact。恢复的 artifact 使用固定 20.94 `A2_latent_residual_adapter` protocol 和 seed `2026`，20.85 baseline 保持 frozen。checkpoint 和 prediction artifact 继续位于 ignored `checkpoints/` 路径，tracked manifest 记录 model config、baseline manifest、normalization、input contract 和 routing contract。

verification 复现 formal A2 metrics：nominal RMSE `0.000335821 m`，non-nominal RMSE `0.000437214 m`，non-nominal Dice `0.842378`。这不更新 `CURRENT_BASELINE.md`，只启用下一步 liftoff-conditioned inference smoke。下一步 route 是测试 live routing：nominal `sensor_z_m≈0.008` 使用 20.85 baseline，non-nominal liftoff 使用 baseline + A2，missing `sensor_z_m` 必须 fail，不能猜测。

# 2026-05-28 Stage 20.96 route note

20.96 把 baseline + A2 companion 结果转成可用 inference path。路线已经明确：`delta_b` 加 mandatory `sensor_z_m` 进入 runner；nominal `0.008 m` 使用 frozen 20.85 baseline，non-nominal liftoff 使用 frozen baseline + A2 latent residual adapter。override modes 只用于 audit：`force_baseline` 和 `force_adapter`。

smoke result 保持 baseline/companion 角色分离。auto routing 让 nominal profile RMSE 保持在 `0.000333059 m`，并复现 A2 non-nominal RMSE `0.000437214 m`；force-baseline non-nominal RMSE 仍是 `0.000874310 m`。`sensor_z_m` 现在是 multi-liftoff 或 real-data inference 的必需 metadata field；超出 `[0.006, 0.012]` 会 flagged，missing liftoff 是 hard error。

这不是 baseline replacement。`CURRENT_BASELINE.md` 仍是 20.85 nominal true 3D RBC profile-depth baseline，A2 仍是 liftoff robustness companion module。下一步应先进入 real-data schema intake 和 acquisition metadata definition，再谈 real-data claims；internal/buried defects 继续 deferred。

# 2026-05-28 Stage 20.97 route note

20.97 把 true 3D RBC 路线从 inference smoke 推到 real-data intake definition。当前 model path 不变：`CURRENT_BASELINE.md` 仍是 20.85 nominal true 3D RBC profile-depth baseline，A2 仍是只有在存在有效 non-nominal `sensor_z_m` metadata 时才启用的 liftoff robustness companion module。

real-data boundary 已经明确。当前路线要求 tri-axis `Bx/By/Bz`、可信 no-defect reference 或 prepared `delta_b`、Tesla units、三条 scan lines、201 x-samples、known axis order、known coordinate system、meter 单位 `sensor_z_m`、sensor alignment status、gain calibration status、specimen/material metadata 和 magnetization setup。缺少 `sensor_z_m`、缺少 reference subtraction、Bz-only input、unknown units/axis order 或 internal/buried defects 都是 blockers。

下一步是 manifest-only dry run，在接受任何 real signal array 前先跑。这样 real-data alignment 会受 acquisition metadata 约束，而不是把 incomplete observations 静默塞进 COMSOL-trained baseline。
...
## 2026-06-03 Stage 25.14 route sync: surface RBC targeted top-up blocker

25.14 把 surface RBC 分支推进到 targeted top-up generation 路线，但它停在 calibration，没有形成可用数据包。120 行计划是 deterministic 且 top-up-only：split `80/20/20`，role coverage `60/24/24/12`，固定 `sensor_z_m=0.008`，tri-axis `Bx/By/Bz`，三条 scan lines，201 个 x points。它不是 assembled training dataset，也不更新 `CURRENT_BASELINE.md`。

真正的路线边界是 calibration failure。`workers=1` calibration 的 24 行 mesh pre-stage 全部通过，随后 COMSOL forward 成功 `22/24`，并出现两个 sample-level geometry failures。因为 calibration 不是 zero-failure，完整 120 top-up generation 被阻塞。唯一有效继续方式是为 `balanced_interior|medium|narrow|sharp|interior` 和 `balanced_interior|deep|balanced|round|interior` 做 same-signature replacement，然后重新跑 calibration。

`+120 training gate` 仍是后续独立阶段。只有在 full top-up 成功并通过 validation 后，那个后续 gate 才可以显式组装 `v3_240 + topup_v1_120` 并创建 assembled manifest。

## 2026-06-03 Stage 25.16 route sync: surface multi-pit label-v3b targets

25.16 把 multi-pit 路线从 label-schema audit 拉回可训练 target gate，但前提是先修正 target semantics。25.15b 的根因不是缺 COMSOL labels：v3 通过扩大 soft/valid support 解决 near-empty masks，却把 component masks 变成 union-like supports，并导致 merged collapse。

v3b 规则是新的分支契约：exclusive hard core 保留 component identity；capped narrow halo 恢复 anti-sparsity；ambiguous overlap/contact pixels 改成 ignore/diagnostic，而不是 multi-positive supervision；depth valid support 跟随 hard core 加 narrow exclusive halo；union mask/depth 保持为 evaluation-only raw OR/max targets。

validator 结果是 `READY_FOR_25_17_TRAINING`：hard-core min `47 px`，soft support min `57 px`，soft OR/raw union mean/max `1.247726 / 1.250000`，v3b/v3 shrink ratio mean `0.627196`，duplicate hard ownership `0`，empty slot violations `0`。当前唯一路线是 25.17 label-v3b training gate，使用 25.10 loss mainline；不要恢复失败的 25.11/25.12 rebalance stack，也不要更新 `CURRENT_BASELINE.md`。

## 2026-06-03 route sync: 25.17 label-v3b training gate

v3b target design 部分修复了 25.13 near-empty failure，但还没有恢复 component separation。在 25.10 loss mainline 且不扩大 architecture 的条件下，25.17 的 test recall 为 `0.674419`，missed `0.325581`，extra `0.292683`，merged `1.000000`，component Dice `0.034007`，union Dice `0.060961`，depth RMSE `0.001178764 m`。

机制是：hard-core / halo / SDF supervision 比 target v2 提供了更多非空 mask support，所以 Dice 高于 25.13；但模型仍塌到 25.15 风格的 union-like merged solution。这不是 baseline candidate，也不能作为回到 25.11/25.12 rebalance stack 的理由。

路线边界：下一阶段原本应是 25.17b label-v3b failure audit，重点检查 loader/loss/metric 是否使用 hard-core identity、halo support、SDF support、depth valid regions 和 overlap ignore masks。25.18 已进一步把主线改为 raster-target route reset；`CURRENT_BASELINE.md` 保持不变。

## 2026-06-03 route sync: 25.18 raster-target route reset

25.18 改变 surface multi-pit 分支判断：per-component raster-target 路线现在停止作为主线。证据已经不是单个 target bug：v2 exclusive ownership 会变成 near-empty，v3 soft support 会变成 union-like，v3b hard-core / halo / SDF 仍让 merged rate 停在 `1.000000`。

保留的分支不是“放弃 multi-pit”，而是做 representation reset。继续保留 `K=3` component slots，但主预测目标转向 geometry：existence、center、`L/W/D`、rotation、shape family 和 compact shape parameters。mask 和 depth map 改为从 geometry slots 派生的结果，用作 evaluator / 弱诊断信号，而不是主 loss。
...
机制是 mixed，而不是整体无效：candidate 在 top-up test 上相对 frozen 20.85 改善了 profile RMSE、Er-like、D MAE 和 projected Dice，但在旧 v3_240 test 的 profile/depth 指标上回退。old-test non-regression 因 profile RMSE `0.000429130307356 m`、Er-like `0.432564256474` 和 D MAE `0.921162613500 mm` 未通过。

路线边界：不更新 `CURRENT_BASELINE.md`，不把 `comsol_true_3d_rbc_surface_expansion_v1_360` 当作 baseline transition，也不要通过 automatic latest/newest discovery 继续推进。任何下一次尝试都必须解释并控制 old-test regression，同时保留 explicit manifest loading 和 validation-only selection。

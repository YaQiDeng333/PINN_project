...
- `results/previews/`
- `results/loss_curves/`
- `results/summaries/` 中保留下来的 29 个代表性文件

代表性 preview、loss curve 和 summary 的具体保留理由见 `ARTIFACT_RETENTION_NOTES.md`。旧 review 文档和大量重复 `.txt` summary 已删除，不作为复现迁移资产。
...
- `notes/`
- `.git/` 以外的本地工具状态
- 旧 checkpoint、旧全量 preview、旧全量 loss curve、旧大计划 CSV、旧 review 文档、未入选的历史 `.txt` summary

"""Create COMSOL polygon inverse subset packages without changing the runner."""
...
def _usage() -> str:
    return (
        "Usage: python make_comsol_polygon_subset_package.py --npz-path INPUT.npz "
        "--polygon-targets polygon_targets.npz --indices 0,1 "
        "--output-npz subset.npz --output-targets subset_polygon_targets.npz"
    )
...
def create_subset(
    ...
    if target_data["presence_targets"].shape[0] != sample_count:
        raise ValueError("NPZ signals and polygon targets sample counts do not match.")
    ...
    subset_npz["subset_source_indices_json"] = np.array(
    ...
    print(f"Saved subset polygon targets to {output_targets}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.npz_path or not args.polygon_targets or not args.indices or not args.output_npz or not args.output_targets:
        print(_usage())
        return 0
    ...
    create_subset(
        Path(args.npz_path),
        Path(args.polygon_targets),
        indices,
        Path(args.output_npz),
        Path(args.output_targets),
    )
    return 0

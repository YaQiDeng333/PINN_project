...
## COMSOL Data-Domain History

旧 COMSOL data-domain baseline 文档已经从根目录移除，不再作为当前导航入口。single-defect、combined single+cc2、two-component multi_defect、three-component multi_defect 这些旧记录只作为历史实验阶段保留在 [EXPERIMENT_LOG.md](EXPERIMENT_LOG.md) 和相关 `results/` 代表性记录中；它们不替代 `CURRENT_BASELINE.md`，也不代表当前 25.19b completion package。

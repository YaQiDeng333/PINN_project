...
DEFAULT_NPZ = PROJECT_ROOT / "data/comsol_mfl/prepared/comsol_multi_defect_three_component_multiline_forward_pack_v3_pilot.npz"
DEFAULT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_three_component_multi_defect_pilot_v3_training_gate_summary.txt"
DEFAULT_AUDIT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_three_component_multi_defect_pilot_v3_failure_audit_summary.txt"
DEFAULT_PLAN = PROJECT_ROOT / "results/summaries/comsol_three_component_multi_defect_pilot_v4_expansion_plan.txt"
DEFAULT_METRICS = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v3_training_gate_metrics.csv"
DEFAULT_EPOCH_LOG = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v3_epoch_log.csv"
DEFAULT_SEED_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v3_seed_summary.csv"
DEFAULT_COMBO_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v3_component_combination_summary.csv"
DEFAULT_CONNECTED_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v3_connected_component_summary.csv"
DEFAULT_FAILURE_CASES = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v3_failure_audit_cases.csv"
DEFAULT_PREVIEW_DIR = PROJECT_ROOT / "results/previews/comsol_three_component_multi_defect_pilot_v3_gate"

SEEDS = [42, 123, 2026]
EXPECTED_SPLITS = {"train": 160, "val": 40, "test": 40}
EXPECTED_COMBOS = {
...
FAILURE_FIELDS = pilot_v1.FAILURE_FIELDS + ["seed", "extra_fragment_flag", "third_largest_component_area_ratio"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run COMSOL three-component multi_defect pilot_v3 3-seed training gate.")
...
def validate_npz(npz_path: Path) -> dict[str, Any]:
    ...
    if delta_bz.shape not in ((240, 3, 201), (160, 3, 201)):
    ...
    if masks.shape not in ((240, 64, 128), (160, 64, 128)):
...
def build_training_summary(context: dict[str, Any]) -> str:
    lines = [
        "# COMSOL true 3-component multi_defect pilot_v3 3-seed training gate",
        "",
        "## Schema",
        "",
        f"- three-component NPZ readable: {context['npz_readable']}",
        f"- schema complete: {context['schema_complete']}",
        f"- split is 160 / 40 / 40 or 108 / 26 / 26: {context['split_ok']} ({context['split_counts']})",
...
def build_audit_summary(context: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# COMSOL true 3-component multi_defect pilot_v3 failure audit",
            "",
...
def build_expansion_plan(context: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# COMSOL 3-component multi_defect pilot_v4 expansion plan",
            "",
            "- Current pack: true 3-component multi_defect pilot_v3 samples with 160 train / 40 val / 40 test for the full 240-sample run.",
            ...
            "- Keep component_count=3 and rectangular_notch/rotated_rect components until this pilot gate is externally reviewed.",
            ...
            "- Do not merge with baseline data or declare a component_count=3 baseline from this pilot gate alone.",
            "- Expansion-readiness condition: schema complete, no data corruption, no systematic missed third component, stable 3-seed val/test behavior, and val/test contain all combinations and distance bins.",
...
def main() -> int:
    ...
    audit_context = {
        **context,
        ...
        "recommendation": (
            "Do not establish a component_count>2 COMSOL baseline from this pilot alone; expand the 3-component data first."

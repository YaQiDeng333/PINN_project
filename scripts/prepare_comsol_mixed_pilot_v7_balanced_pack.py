#!/usr/bin/env python
"""Prepare the balanced COMSOL mixed pilot_v7 pack.

This script combines existing rectangular_notch / rotated_rect / polygon COMSOL
pilot packs with the real pilot_v7 top-up samples. It does not run COMSOL and
...
DEFAULT_OUTPUT_NPZ = (
    PROJECT_ROOT
    / "data/comsol_mfl/prepared/comsol_single_defect_multiline_forward_pack_v1_pilot_v7_balanced_three_types.npz"
)
DEFAULT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_mixed_pilot_v7_balanced_pack_summary.txt"
DEFAULT_INVENTORY = PROJECT_ROOT / "results/metrics/comsol_mixed_pilot_v7_balanced_inventory.csv"
TOPUP_MANIFEST_NAME = "manifest_comsol_single_defect_multiline_forward_pack_v1_pilot_v7_topup.csv"

TARGET_PER_TYPE = 80
SPLIT_TARGET_PER_TYPE = {"train": 54, "val": 13, "test": 13}
TARGET_TOTAL_SPLITS = {"train": 162, "val": 39, "test": 39}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare balanced COMSOL mixed pilot_v7 pack.")
    ...
    parser.add_argument("--topup-dir", type=Path, default=DEFAULT_TOPUP_DIR)
...
def load_topup_samples(topup_dir: Path) -> list[dict[str, Any]]:
    manifest_path = topup_dir / TOPUP_MANIFEST_NAME
    ...
    for row in rows:
        ...
        source_pack = raw_geometry.get("source_pack", f"pilot_v7_topup_{defect_type}")
...
def assign_ids(samples_by_type: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
    prefixes = {"rectangular_notch": "rect", "rotated_rect": "rot", "polygon": "poly"}
    ...
    for defect_type in ["rectangular_notch", "rotated_rect", "polygon"]:
        for seq, sample in enumerate(samples_by_type[defect_type], start=1):
            sample["sample_id"] = f"{prefixes[defect_type]}_{seq:04d}"
...
def inventory_row(sample: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "notes": "balanced pilot_v7 single-defect mixed sample",
    }
...
def validate_merged(samples: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if dict(defect_counts) != {"rectangular_notch": 80, "rotated_rect": 80, "polygon": 80}:
    ...
    if delta_bz.shape != (240, 3, 201) or masks.shape != (240, 64, 128):
...
def main() -> int:
    ...
    topup_dir = resolve(args.topup_dir)
    ...
    topup_samples = load_topup_samples(topup_dir)
    check_topup_coordinates(topup_samples, rect_pack)
    topup_counts = Counter(sample["defect_type"] for sample in topup_samples)
    if dict(topup_counts) != {"rotated_rect": 32, "polygon": 20}:
        raise PackValidationError(f"top-up distribution mismatch: {dict(topup_counts)}")

    rect_samples = build_existing_samples(rect_pack, "pilot_v2_rectangular_notch", "rectangular_notch", select_evenly(120, 80))
    ...
    if len(rect_samples) != 80 or len(rotated_samples) != 80 or len(polygon_samples) != 80:
    ...
    split_samples(rotated_samples, lambda item: float(item["geometry"]["angle_deg"]))
    split_samples(polygon_samples, lambda item: int(item["geometry"]["vertex_count"]))
    samples = assign_ids(
        {
    ...
    metadata = {
        "pack_name": "comsol_single_defect_multiline_forward_pack_v1_pilot_v7_balanced_three_types",
        "source_packs": [str(rect_npz), str(rot_npz), str(poly_npz), str(topup_dir / TOPUP_MANIFEST_NAME)],
        ...
        "note": "balanced pilot_v7 mixed single-defect-type pack; not official full dataset and not multi_defect",
    }
    if not args.dry_run:
        ...
        ensure_source_pack("pilot_v7_balanced", merged, 240, "rectangular_notch") if False else None
        if merged["delta_bz"].shape != (240, 3, 201) or merged["masks"].shape != (240, 64, 128):
    ...
    summary_lines = [
        "# COMSOL mixed pilot_v7 balanced pack summary",
        "",
        f"source packs readable: yes ({rect_npz}, {rot_npz}, {poly_npz})",
        f"top-up readable: yes ({topup_dir / TOPUP_MANIFEST_NAME})",
        "coordinate consistency: yes (sensor_x, scan_line_y, mask_x, mask_y all match)",
        f"balanced pilot_v7 NPZ generated: {'no, dry-run only' if args.dry_run else 'yes'}",
        f"balanced pilot_v7 NPZ path: {output_npz}",
        ...
        "schema-ready: yes",
        "pilot_v7 train-ready: yes, for balanced pilot smoke/training gate only",
        "current limitations: still pilot-level, single-defect only, no multi_defect, limited COMSOL geometry families",
        "next step: enter balanced pilot_v7 training gate in PINN_project",

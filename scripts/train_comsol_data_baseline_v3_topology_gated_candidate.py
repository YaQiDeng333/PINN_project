...
DEFAULT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_data_baseline_v3_topology_gated_poc_summary.txt"
DEFAULT_AUDIT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_data_baseline_v3_topology_gated_poc_failure_audit_summary.txt"
DEFAULT_METRICS = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_topology_gated_poc_metrics.csv"
DEFAULT_EPOCH_LOG = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_topology_gated_poc_epoch_log.csv"
DEFAULT_SEED_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_topology_gated_poc_seed_summary.csv"
DEFAULT_DEFECT_GROUP_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_topology_gated_poc_defect_group_summary.csv"
DEFAULT_TASK_GROUP_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_topology_gated_poc_task_group_summary.csv"
DEFAULT_CONNECTED_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_topology_gated_poc_connected_component_summary.csv"
DEFAULT_COMPONENT_COUNT_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_topology_gated_poc_component_count_summary.csv"
DEFAULT_GATE_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_topology_gated_poc_gate_summary.csv"
DEFAULT_FAILURE_CASES = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_topology_gated_poc_failure_cases.csv"
DEFAULT_PREVIEW_DIR = PROJECT_ROOT / "results/previews/comsol_data_baseline_v3_topology_gated_poc"
...
class TopologyBranchDecoder(torch.nn.Module):
...
class TopologyGatedMaskDecoder(torch.nn.Module):
    def __init__(self, n_lines: int, mask_h: int, mask_w: int):
        super().__init__()
        if (mask_h, mask_w) != (64, 128):
            raise ValueError("Topology-gated decoder expects 64x128 masks.")
        ...
        self.decoders = torch.nn.ModuleList([TopologyBranchDecoder(512) for _ in TASK_LABELS])
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        branch_logits = torch.stack([decoder(latent) for decoder in self.decoders], dim=1)
        mask_logits = (branch_logits * gate_probs[:, :, None, None]).sum(dim=1)
        return {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train COMSOL_DATA_BASELINE_V3 topology-aware gated decoder candidate.")
...
def gated_loss_components(
    ...
    total = mask_total + LAMBDA_TASK * task_ce + LAMBDA_COUNT * count_ce
    return total, bce, dice, task_ce, count_ce
...
def evaluate_gated_loss(
    ...
    with torch.no_grad():
        for signals, targets, indices in loader:
            ...
            total, bce, dice, task_ce, count_ce = gated_loss_components(outputs, targets, task_labels, count_labels)
...
def evaluate_model(
    ...
    with torch.no_grad():
        for signals, masks, indices in loader:
            ...
            task_labels, count_labels = label_tensors(indices, validation, device)
            outputs = model(signals)
            total, bce, dice, task_ce, count_ce = gated_loss_components(outputs, masks, task_labels, count_labels)
            ...
            rows.append(
                {
                    ...
                    **metrics,
                    ...
                    "notes": "topology_gated_candidate_eval",
                }
            )
    return rows, probs
...
def train_one_seed(seed: int, args: argparse.Namespace, validation: dict[str, Any], device: torch.device) -> dict[str, Any]:
    ...
    model = TopologyGatedMaskDecoder(delta_bz.shape[1], masks.shape[1], masks.shape[2]).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_total, val_bce, val_dice, val_task_ce, val_count_ce = evaluate_gated_loss(model, val_dataset, device, validation)
...
def build_training_summary(context: dict[str, Any]) -> str:
    lines = [
        ...
        f"- model: shared Conv1d BzEncoder + predicted task gate + 3 topology-specific decoders, delta_bz input only",
        f"- auxiliary loss: task_group CE lambda={LAMBDA_TASK}, component_count CE lambda={LAMBDA_COUNT}",
...
def main() -> int:
    ...
    cc2_improved = cc2_iou_delta >= 0.015 or cc2_dice_delta >= 0.012
    ...
    single_ok = single_iou_delta >= -0.02

#!/usr/bin/env python
"""Assemble v1 source + 20.74 top-up into the v2_120 RBC dataset candidate."""
...
DEFAULT_SOURCE = ROOT / "data/comsol_mfl/prepared/experimental/true_3d_rbc_pilot/comsol_true_3d_rbc_imported_watertight_pilot_v1_assembled.npz"
...
DEFAULT_SUMMARY = ROOT / "results/summaries/true_3d_rbc_dataset_120_assembled_summary.txt"
DEFAULT_INDEX = ROOT / "results/metrics/true_3d_rbc_dataset_120_assembled_index.csv"
DEFAULT_GROUPS = ROOT / "results/metrics/true_3d_rbc_dataset_120_assembled_group_summary.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Assemble v1 + 20.74 top-up into v2_120.")
...
def metrics(pack: dict[str, np.ndarray], source_labels: list[str]) -> tuple[list[dict[str, Any]], bool]:
    ...
    depths = [str(x) for x in pack["depth_bin"]]
    sizes = [str(x) for x in pack["size_bin"]]
    aspects = [str(x) for x in pack["aspect_bin"]]
...
def train_ready(rows: list[dict[str, Any]], validation_pass: bool) -> bool:
    ...
    return (
        validation_pass
        and len(rows) >= 108
        and split.get("train", 0) >= 72
        and split.get("val", 0) >= 18
        and split.get("test", 0) >= 18
        and all(curv.get(name, 0) >= 20 for name in ["sharp", "round", "boxy", "LD_dominant", "WD_dominant"])
    )
...
def run(args: argparse.Namespace) -> int:
    ...
    if scalar_str(source["dataset_id"]) != SOURCE_ID:
        raise RuntimeError("source NPZ dataset_id mismatch")
    if scalar_str(topup["dataset_id"]) != TOPUP_ID:
        raise RuntimeError("top-up NPZ dataset_id mismatch")
    source_labels = ["v1_assembled_source"] * len(source["sample_ids"]) + ["v2_topup_20_74"] * len(topup["sample_ids"])
    ...
    lines = [
        "20.74 true 3D RBC v2_120 assembled summary",
        "",

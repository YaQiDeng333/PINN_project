...
REFINE_FOURIER_FREQUENCIES = (1.0, 2.0, 4.0, 8.0)

CHECKPOINT_DIR = ROOT / 'checkpoints' / 'mask_boundary_grid_refine_candidate'
METRICS_PATH = ROOT / 'results' / 'metrics' / 'v3_complex_mask_boundary_grid_refine_candidate_metrics.csv'
SUMMARY_PATH = ROOT / 'results' / 'summaries' / 'v3_complex_mask_boundary_grid_refine_candidate_summary.txt'
PREVIEW_DIR = ROOT / 'results' / 'previews' / 'mask_boundary_grid_refine_candidate'
...
class MaskBoundaryGridRefineModel(nn.Module):
    def __init__(
        self,
        signal_length,
        signal_channels=1,
        ...
        low_shape=GRID_LOW_SHAPE,
        base_channels=GRID_BASE_CHANNELS,
        fourier_frequencies=REFINE_FOURIER_FREQUENCIES,
    ):
        super().__init__()
        self.out_shape = tuple(out_shape)
        self.low_shape = tuple(low_shape)
        self.base_channels = int(base_channels)
        self.fourier_frequencies = tuple(float(value) for value in fourier_frequencies)
        self.bz_encoder = BzEncoder(
            signal_length=signal_length,
            signal_channels=signal_channels,
            latent_dim=latent_dim,
        ...
        coord_channels = 2 + 4 * len(self.fourier_frequencies)
        self.refine_head = nn.Sequential(
            nn.Conv2d(16 + coord_channels, 32, kernel_size=3, padding=1),
            nn.GroupNorm(8, 32),
            nn.SiLU(),
    ...
    def coordinate_channels(self, batch_size, device):
        ...
        for freq in self.fourier_frequencies:
            channels.extend([
                torch.sin(np.pi * freq * xx),
                torch.cos(np.pi * freq * xx),
                torch.sin(np.pi * freq * yy),
                torch.cos(np.pi * freq * yy),
            ])
        coords = torch.stack(channels, dim=0).unsqueeze(0)
    ...
    def forward(self, bz_signal, coords=None):
        ...
        low_h, low_w = self.low_shape
        features = self.project(bz_latent).view(batch_size, self.base_channels, low_h, low_w)
        grid_features = self.decoder_body(features)
        if tuple(grid_features.shape[-2:]) != self.out_shape:
            grid_features = F.interpolate(grid_features, size=self.out_shape, mode='bilinear', align_corners=False)
        coord_features = self.coordinate_channels(batch_size, bz_signal.device)
        logits = self.refine_head(torch.cat([grid_features, coord_features], dim=1))
        return logits[:, 0].reshape(batch_size, -1)
...
def load_refine_checkpoint(path, signal_length, signal_channels, out_shape, device):
    checkpoint = torch.load(path, map_location=device)
    args = checkpoint.get('args', {})
    model = MaskBoundaryGridRefineModel(
        signal_length=signal_length,
        signal_channels=int(args.get('signal_channels', signal_channels)),
        latent_dim=int(args.get('latent_dim', LATENT_DIM)),
        out_shape=tuple(args.get('out_shape', out_shape)),
        low_shape=tuple(args.get('low_shape', GRID_LOW_SHAPE)),
        base_channels=int(args.get('base_channels', GRID_BASE_CHANNELS)),
        fourier_frequencies=tuple(args.get('fourier_frequencies', REFINE_FOURIER_FREQUENCIES)),
    ).to(device)
    model.load_state_dict(checkpoint['model_state_dict'])
    model.eval()
    return model, checkpoint
...
def train_one_seed(seed, device, pos_weight_value):
    ...
    best_info = None
    best_path = CHECKPOINT_DIR / f'best_mask_boundary_grid_refine_seed{seed}.pt'
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if selection_score > best_score:
            ...
            torch.save({
                ...
                'args': {
                    'model': 'mask_boundary_grid_refine_model',
                    'dataset': 'v3_complex',
                    'seed': seed,
                    'epochs': EPOCHS,
                    'batch_size': BATCH_SIZE,
                    ...
                    'decoder': 'latent MLP projection to low-resolution feature map plus ConvTranspose2d upsampling and coordinate refinement conv head',
                    'out_shape': out_shape,
                    'low_shape': GRID_LOW_SHAPE,
                    'base_channels': GRID_BASE_CHANNELS,
                    'fourier_frequencies': REFINE_FOURIER_FREQUENCIES,
...
def evaluate_checkpoint_family(checkpoints, model_type, candidate, split, data_path, thresholds, device, area_edges, low_signal_indices):
    ...
    for seed, checkpoint_path in checkpoints.items():
        ...
        if model_type == 'grid':
            ...
            model, _ = load_refine_checkpoint(Path(project_path(checkpoint_path)), signal_length, signal_channels, out_shape, device)
        else:
...
def select_threshold(rows, baseline_overall):
    validation_means = [
        row for row in rows
        if row['candidate'] == 'mask_boundary_grid_refine_val_scan_mean'
        and row['split'] == 'val'
        and row['group_type'] == 'overall'
        and row['group'] == 'all'
    ]
...
def format_comparison_table(rows, group_type, groups, selected_threshold):
    ...
    for group in groups:
        ...
        grid_mean = find_row(rows, 'mask_boundary_grid_refine_test_mean', group_type, group, threshold=selected_threshold)
        grid_std = find_row(rows, 'mask_boundary_grid_refine_test_std', group_type, group, threshold=selected_threshold)
        lines.append(
        ...
        lines.append(
            f"| {group} | grid + coordinate refinement | {selected_threshold:.2f} | "
...
def improvement_status(rows, group_type, group, selected_threshold):
    ...
    candidate = find_row(rows, 'mask_boundary_grid_refine_test_mean', group_type, group, threshold=selected_threshold)
    return {
...
def write_summary(rows, best_infos, checkpoint_paths, selected_threshold, pos_weight, mask_fraction):
    ...
    summary = f"""# v3_complex mask-only grid decoder + coordinate refinement candidate

This RESULT_DRIVEN_EXPERIMENT trains an independent mask-only grid decoder + coordinate refinement candidate for seeds 42, 123, and 2026. It does not modify train_pinn.py, evaluate_pinn.py, data_generator_v2.py, CURRENT_BASELINE.md, README.md, EXPERIMENT_LOG.md, the route document, or NEXT_STEP.md.

## Model and loss

The model keeps the direct Bz -> mask setup. It reuses BzEncoder, projects the latent vector into a low-resolution 2D feature map, upsamples it with ConvTranspose2d blocks, then concatenates normalized coordinate channels and fixed Fourier coordinate channels before a lightweight convolution refinement head outputs full-grid mask logits.

* BzEncoder latent dimension: {LATENT_DIM}
* low-resolution feature map: {GRID_BASE_CHANNELS} x {GRID_LOW_SHAPE[0]} x {GRID_LOW_SHAPE[1]}
* coordinate channels: x, y, and fixed Fourier frequencies {', '.join(f'{value:g}' for value in REFINE_FOURIER_FREQUENCIES)}
* refinement head: Conv2d blocks over grid features + coordinate features

Training loss is BCEWithLogits + soft Dice. No SDF, boundary head/loss, loss-weight sweep, adaptive threshold, post-processing, or ensemble is used.
...
def select_preview_samples(grid_means, baseline_means):
    ...
    take('mask_boundary_grid_refine_failure', failures, 3)
...
def main():
    ...
    best_infos = []
    for seed in SEEDS:
        print(f'Training mask boundary grid + coordinate refinement candidate seed={seed}')
    ...
    val_rows, _, _ = evaluate_checkpoint_family(
        ...
        model_type='refine',
        candidate='mask_boundary_grid_refine_val_scan',
        split='val',
        data_path=VAL_DATA,
    ...
    test_rows, grid_sample_rows, grid_prob_cache = evaluate_checkpoint_family(
        checkpoints=grid_checkpoints,
        model_type='refine',
        candidate='mask_boundary_grid_refine_test',
        split='test',
        data_path=TEST_DATA,

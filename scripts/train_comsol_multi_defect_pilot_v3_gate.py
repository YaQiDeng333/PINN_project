...
DEFAULT_NPZ = PROJECT_ROOT / "data/comsol_mfl/prepared/comsol_multi_defect_multiline_forward_pack_v3_pilot.npz"
DEFAULT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_multi_defect_pilot_v3_training_gate_summary.txt"
DEFAULT_AUDIT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_multi_defect_pilot_v3_failure_audit_summary.txt"
DEFAULT_PLAN = PROJECT_ROOT / "results/summaries/comsol_multi_defect_baseline_readiness_plan.txt"
DEFAULT_METRICS = PROJECT_ROOT / "results/metrics/comsol_multi_defect_pilot_v3_training_gate_metrics.csv"
DEFAULT_EPOCH_LOG = PROJECT_ROOT / "results/metrics/comsol_multi_defect_pilot_v3_epoch_log.csv"
DEFAULT_SEED_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_multi_defect_pilot_v3_seed_summary.csv"
DEFAULT_COMBO_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_multi_defect_pilot_v3_component_combination_summary.csv"
DEFAULT_CONNECTED_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_multi_defect_pilot_v3_connected_component_summary.csv"
DEFAULT_FAILURE_CASES = PROJECT_ROOT / "results/metrics/comsol_multi_defect_pilot_v3_failure_audit_cases.csv"
DEFAULT_PREVIEW_DIR = PROJECT_ROOT / "results/previews/comsol_multi_defect_pilot_v3_gate"

SEEDS = [42, 123, 2026]
EXPECTED_SPLITS = {"train": 160, "val": 40, "test": 40}
EXPECTED_COMBOS = {
    "rectangular_notch+polygon",
...
class MultiDefectV3ValidationError(RuntimeError):
    pass


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run COMSOL multi_defect pilot_v3 3-seed training gate.")
...
def validate_npz(npz_path: Path) -> dict[str, Any]:
    ...
    if missing:
        raise MultiDefectV3ValidationError(f"missing NPZ fields: {missing}")
    ...
    if delta_bz.shape != (240, 3, 201):
        raise MultiDefectV3ValidationError(f"unexpected delta_bz shape: {delta_bz.shape}")
    if bz_defect.shape != delta_bz.shape or bz_no_defect.shape != delta_bz.shape:
        raise MultiDefectV3ValidationError("bz_defect / bz_no_defect shape mismatch")
    if masks.shape != (240, 64, 128):
        raise MultiDefectV3ValidationError(f"unexpected masks shape: {masks.shape}")
    if sensor_x.shape != (201,) or scan_line_y.shape != (3,) or mask_x.shape != (128,) or mask_y.shape != (64,):
        raise MultiDefectV3ValidationError("coordinate shape mismatch")
    if not (
        ...
        raise MultiDefectV3ValidationError("coordinates are not strictly increasing")
    if not all(np.isfinite(array).all() for array in (delta_bz, bz_defect, bz_no_defect, masks, sensor_x, scan_line_y, mask_x, mask_y)):
        raise MultiDefectV3ValidationError("non-finite arrays found")
    if not np.allclose(delta_bz, bz_defect - bz_no_defect, rtol=1e-9, atol=1e-12):
        raise MultiDefectV3ValidationError("delta_bz mismatch")
    if float(delta_bz.std()) <= 0.0:
        raise MultiDefectV3ValidationError("delta_bz is zero")
    if np.max(np.abs(delta_bz[:, 1:, :] - delta_bz[:, :1, :])) <= 1e-12:
        raise MultiDefectV3ValidationError("scan lines are identical")
    if not np.all(masks.reshape(masks.shape[0], -1).sum(axis=1) > 0):
        raise MultiDefectV3ValidationError("empty mask found")
    if set(defect_types.tolist()) != {"multi_defect"}:
        raise MultiDefectV3ValidationError(f"unexpected defect_types: {Counter(defect_types.tolist())}")
    if not np.all(component_counts == 2):
        raise MultiDefectV3ValidationError(f"component_counts not all 2: {Counter(component_counts.tolist())}")
    if not np.all(connected_counts == 2):
        raise MultiDefectV3ValidationError(f"connected_component_counts not all 2: {Counter(connected_counts.tolist())}")
    if len(set(sample_ids.tolist())) != len(sample_ids):
        raise MultiDefectV3ValidationError("sample_ids are not unique")
    split_counts = Counter(splits.tolist())
    if dict(split_counts) != EXPECTED_SPLITS:
        raise MultiDefectV3ValidationError(f"unexpected split counts: {dict(split_counts)}")
    ...
    for index, component_list in enumerate(components):
        if not isinstance(component_list, list) or len(component_list) != 2:
            raise MultiDefectV3ValidationError(f"components_json index {index} is not a 2-component list")
        ...
        if combo not in EXPECTED_COMBOS:
            raise MultiDefectV3ValidationError(f"unexpected component combo: {combo}")
        ...
        for component in component_list:
            ...
            if int(component_mask.sum()) <= 0:
                raise MultiDefectV3ValidationError(f"empty component mask at sample {index}")
        ...
        if cc != 2:
            raise MultiDefectV3ValidationError(f"mask connected component count at {index} is {cc}")
        min_distances.append(float(math.hypot(centers[0][0] - centers[1][0], centers[0][1] - centers[1][1])))
    if min(geometry_mask_ious) < 0.999:
        raise MultiDefectV3ValidationError(f"components_json does not explain masks: min IoU {min(geometry_mask_ious)}")
...
def train_one_seed(
    ...
    for row in metric_rows:
        ...
        row["polygon_component_present"] = "yes" if "polygon" in row["component_types"] else "no"
...
def build_training_summary(context: dict[str, Any]) -> str:
    lines = [
        "# COMSOL true multi_defect pilot_v3 3-seed training gate",
        "",
        "## Schema",
        "",
        f"- pilot_v3 NPZ readable: {context['npz_readable']}",
        f"- schema complete: {context['schema_complete']}",
        f"- split is 160 / 40 / 40: {context['split_ok']} ({context['split_counts']})",
        ...
        f"- can fit 160 train samples: {context['can_fit_train_samples']}",
        "",
        ...
        f"- polygon vs non-polygon test Dice: {context['polygon_vs_nonpolygon']}",
        ...
        "## Conclusion",
        "",
        "- This is a multi_defect pilot_v3 gate only, not a baseline and not a v3_complex comparison.",
        f"- recommend expanding multi_defect data: {context['recommend_expand']}",
        f"- polygon component clearly harder: {context['polygon_component_harder']}",
...
def build_audit_summary(context: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# COMSOL true multi_defect pilot_v3 failure audit",
            "",
...
def build_expansion_plan(context: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# COMSOL multi_defect baseline readiness plan",
            "",
            "- Current pack: 240 true multi_defect pilot_v3 samples with 160 train / 40 val / 40 test.",
            "- Component combinations include rectangular_notch+rectangular_notch, rectangular_notch+rotated_rect, rotated_rect+rotated_rect, rectangular_notch+polygon, and rotated_rect+polygon.",
            "- Keep component_count=2 until this pilot_v3 gate is externally reviewed.",
            ...
            "- Do not merge with single-defect data or declare a multi_defect baseline until this pilot_v3 gate is reviewed.",
            "- Baseline-readiness condition: schema complete, no data corruption, no systematic missed second component, stable 3-seed val/test behavior, and val/test contain all combinations and distance bins.",
            ...
            f"- Current pilot_v3 signal: {context['recommendation']}",
        ]
    ) + "\n"
...
def main() -> int:
    ...
    component_rows.extend(build_group_rows(all_metric_rows, "polygon_component_present", ["no", "yes"]))
    ...
    polygon_test = [row for row in test_rows if row["polygon_component_present"] == "yes"]
    non_polygon_test = [row for row in test_rows if row["polygon_component_present"] == "no"]
    polygon_component_harder = False
    polygon_vs_nonpolygon = {"polygon_test_dice": None, "non_polygon_test_dice": None}
    if polygon_test and non_polygon_test:
        polygon_dice = float(np.mean([float(row["dice"]) for row in polygon_test]))
        non_polygon_dice = float(np.mean([float(row["dice"]) for row in non_polygon_test]))
        polygon_vs_nonpolygon = {"polygon_test_dice": polygon_dice, "non_polygon_test_dice": non_polygon_dice}
        polygon_component_harder = polygon_dice + 0.05 < non_polygon_dice
    recommend_expand = True
    recommend_baseline = test_missed_rate < 0.10 and test_merged_rate < 0.10 and test_cc2_rate > 0.80
    ...
    context = {
        ...
        "failure_main": (
            "Pilot_v3 is trainable and component diagnostics are meaningful; dominant failures remain "
        ...
        "polygon_component_harder": polygon_component_harder,
        "polygon_vs_nonpolygon": polygon_vs_nonpolygon,
        "limitations": "pilot_v3 only, component_count=2 only, no polygon+polygon, no real experimental data, not a baseline",
    }
    audit_context = {
        **context,
        ...
        "recommendation": (
            "Pilot_v3 is a candidate for external review before deciding whether to establish a multi_defect COMSOL data-domain baseline."
            if recommend_baseline
            else "Do not establish a multi_defect baseline yet; expand or inspect pilot_v3 failure modes first."
        ),
    }

...
DEFAULT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_data_baseline_v3_enhanced_candidate_training_summary.txt"
DEFAULT_AUDIT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_data_baseline_v3_enhanced_candidate_failure_audit_summary.txt"
DEFAULT_METRICS = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_enhanced_candidate_metrics.csv"
DEFAULT_EPOCH_LOG = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_enhanced_candidate_epoch_log.csv"
DEFAULT_SEED_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_enhanced_candidate_seed_summary.csv"
DEFAULT_DEFECT_GROUP_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_enhanced_candidate_defect_group_summary.csv"
DEFAULT_TASK_GROUP_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_enhanced_candidate_task_group_summary.csv"
DEFAULT_DEFECT_TYPE_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_enhanced_candidate_defect_type_summary.csv"
DEFAULT_CONNECTED_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_enhanced_candidate_connected_component_summary.csv"
DEFAULT_COMPONENT_COUNT_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_enhanced_candidate_component_count_summary.csv"
DEFAULT_COMPONENT_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_enhanced_candidate_component_combination_summary.csv"
DEFAULT_SOURCE_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_enhanced_candidate_source_summary.csv"
DEFAULT_FAILURE_CASES = PROJECT_ROOT / "results/metrics/comsol_data_baseline_v3_enhanced_candidate_failure_cases.csv"
DEFAULT_PREVIEW_DIR = PROJECT_ROOT / "results/previews/comsol_data_baseline_v3_enhanced_candidate"
...
class EnhancedComsolMaskDecoder(torch.nn.Module):
    def __init__(self, n_lines: int, mask_h: int, mask_w: int):
        super().__init__()
        if (mask_h, mask_w) != (64, 128):
            raise ValueError("Enhanced decoder expects 64x128 masks.")
        self.encoder = torch.nn.Sequential(
            torch.nn.Conv1d(n_lines, 64, kernel_size=7, padding=3),
        ...
        self.project = torch.nn.Sequential(
            torch.nn.Linear(256 * 16, 1024),
            torch.nn.GELU(),
            torch.nn.Dropout(p=0.05),
            torch.nn.Linear(1024, 256 * 4 * 8),
            torch.nn.GELU(),
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        feature = self.project(latent).view(x.shape[0], 256, 4, 8)
        return self.decoder(feature).squeeze(1)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train COMSOL_DATA_BASELINE_V3 enhanced-capacity candidate.")
...
def train_one_seed(seed: int, args: argparse.Namespace, validation: dict[str, Any], device: torch.device) -> dict[str, Any]:
    ...
    model = EnhancedComsolMaskDecoder(delta_bz.shape[1], masks.shape[1], masks.shape[2]).to(device)
...
def build_training_summary(context: dict[str, Any]) -> str:
    lines = [
        "# COMSOL_DATA_BASELINE_V3 enhanced-capacity candidate training summary",
        "",
        ...
        f"- model: enhanced-capacity mask-only Conv1d/BzEncoder + grid decoder, delta_bz input only",
        f"- capacity change: {context['capacity_change']}",
        ...
        f"- cc2 improvement over 20.42: {context['cc2_improvement']}",
        f"- cc2 pred_cc_is_2 did not decrease: {context['cc2_pred_not_decrease']}",
        f"- single_defect within degradation tolerance: {context['single_tradeoff_ok']}",
        f"- cc3 within degradation tolerance: {context['cc3_tradeoff_ok']}",
        f"- source_pack collapse: {context['source_issue']}",
        f"- unstable seed collapse: {context['seed_collapse']}",
        f"- enhanced acceptance passed: {context['enhanced_acceptance_passed']}",
        "",
        ...
        f"- can proceed to COMSOL_DATA_BASELINE_V3 enhanced candidate documentation: {context['enhanced_acceptance_passed']}",
        "",
        "## Limitations",
        "",
...
def build_audit_summary(context: dict[str, Any]) -> str:
    lines = [
        "# COMSOL_DATA_BASELINE_V3 enhanced-capacity candidate failure audit",
        "",
        f"- cc2 improved vs 20.42 lightweight candidate: {context['cc2_improved']}",
        f"- cc2 pred_cc_is_2 did not decrease: {context['cc2_pred_not_decrease']}",
        f"- single_defect trade-off acceptable: {context['single_tradeoff_ok']}",
        f"- cc3 trade-off acceptable: {context['cc3_tradeoff_ok']}",
        ...
        f"- enhanced candidate passes acceptance check: {context['enhanced_acceptance_passed']}",
        "",
        "## Interpretation",
        "",
...
def main() -> int:
    ...
    cc2_improved = cc2_iou_delta >= 0.02 or cc2_dice_delta >= 0.015
    cc2_pred_delta = cc2_pred_cc2 - LIGHTWEIGHT_V3_REFERENCE["cc2_pred_cc_is_2"]
    cc2_pred_not_decrease = cc2_pred_delta >= -1e-9
    single_iou_delta = float(single_test["iou_mean"]) - LIGHTWEIGHT_V3_REFERENCE["single_iou"]
    single_dice_delta = float(single_test["dice_mean"]) - LIGHTWEIGHT_V3_REFERENCE["single_dice"]
    single_tradeoff_ok = single_iou_delta >= -0.015 and single_dice_delta >= -0.015
    cc3_iou_delta = float(cc3_test["iou_mean"]) - LIGHTWEIGHT_V3_REFERENCE["cc3_iou"]
    cc3_dice_delta = float(cc3_test["dice_mean"]) - LIGHTWEIGHT_V3_REFERENCE["cc3_dice"]
    cc3_tradeoff_ok = cc3_iou_delta >= -0.015 and cc3_dice_delta >= -0.015
    seed_collapse = False
    for seed in SEEDS:
        seed_test_rows = [row for row in all_metric_rows if row["split"] == "test" and int(row["seed"]) == seed]
        ...
        if mean_or_nan([float(row["dice"]) for row in seed_cc3_rows]) < LIGHTWEIGHT_V3_REFERENCE["cc3_dice"] - 0.05:
            seed_collapse = True
    enhanced_acceptance_passed = (
        cc2_improved
        and cc2_pred_not_decrease
        and single_tradeoff_ok
        and cc3_tradeoff_ok
        and not source_issue
    ...
    context = {
        ...
        "capacity_change": "Conv1d channels widened from 32/64/128 to 64/128/256 with a wider 256-channel grid decoder; no metadata, component, or auxiliary heads.",
        ...
        "cc2_pred_not_decrease": cc2_pred_not_decrease,
        "single_tradeoff_ok": single_tradeoff_ok,
        "cc3_tradeoff_ok": cc3_tradeoff_ok,
        "seed_collapse": seed_collapse,
        "enhanced_acceptance_passed": enhanced_acceptance_passed,
        ...
        "interpretation": (
            "The enhanced-capacity candidate passes the Stage 20.43 acceptance check and can be recorded as a candidate for human review."
            if enhanced_acceptance_passed
            else "The enhanced-capacity candidate does not pass the Stage 20.43 acceptance check; do not document it as COMSOL_DATA_BASELINE_V3."
        ),
    }
        ...
        json.dumps(
            {
                ...
                "enhanced_acceptance_passed": context["enhanced_acceptance_passed"],

#!/usr/bin/env python
"""Validate and register the 21.0 internal / buried defect COMSOL smoke pack.
...
PACK_NPZ = ROOT / "data/comsol_mfl/generated/internal_defect_smoke_pack/internal_defect_smoke_pack_v1.npz"
COMSOL_INVENTORY = Path(r"C:\Users\19166\Desktop\COMSOL_Multiphysics_MCP\results\inventory_internal_defect_smoke_pack.csv")
COMSOL_SUMMARY = Path(r"C:\Users\19166\Desktop\COMSOL_Multiphysics_MCP\results\internal_defect_smoke_pack_summary.txt")
SUMMARY = ROOT / "results/summaries/internal_defect_smoke_pack_validation_summary.txt"
METRICS = ROOT / "results/metrics/internal_defect_smoke_pack_validation_metrics.csv"
...
REGISTRY_SUMMARY = ROOT / "results/summaries/internal_defect_smoke_pack_registry_summary.txt"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate and register internal defect smoke pack.")
...
def status_from_success(success_count: int, planned_count: int, arrays: dict[str, np.ndarray]) -> str:
    if success_count == planned_count and planned_count == 12:
        return "smoke_generated"
    if success_count >= 6:
        return "partial_smoke_generated"
    return "blocked"
...
def run(args: argparse.Namespace) -> int:
    ...
    add(checks, "plan_csv_exists", args.plan_csv.exists(), str(args.plan_csv), "internal smoke plan CSV")
    add(checks, "planned_rows_12", len(plan_rows) == 12, len(plan_rows), 12)
    ...
    add(checks, "success_count_minimum", success_count >= 6, success_count, ">=6")
    ...
    if arrays:
        ...
        if delta is not None and b_defect is not None and b_no is not None and delta.shape == b_defect.shape == b_no.shape:
            max_error = float(np.max(np.abs(delta - (b_defect - b_no)))) if delta.size else float("nan")
            add(checks, "delta_matches_defect_minus_reference", max_error <= 1.0e-6, max_error, "<=1e-6")
        axis_names = as_strings(arrays.get("axis_names", np.asarray([])))
        add(checks, "axis_names", axis_names == AXIS_NAMES, axis_names, AXIS_NAMES)
        sensor_x = arrays.get("sensor_x")
        add(checks, "sensor_x_count", sensor_x is not None and int(np.asarray(sensor_x).reshape(-1).shape[0]) == 201, getattr(sensor_x, "shape", None), 201)
        scan_line_y = arrays.get("scan_line_y")
        if scan_line_y is not None:
            observed_lines = np.asarray(scan_line_y, dtype=float)
            if observed_lines.ndim == 2:
                line_ok = bool(np.allclose(observed_lines, np.asarray(SCAN_LINE_Y, dtype=float)[None, :]))
            else:
                line_ok = bool(np.allclose(observed_lines.reshape(-1), np.asarray(SCAN_LINE_Y, dtype=float)))
            add(checks, "scan_line_y", line_ok, observed_lines.tolist(), SCAN_LINE_Y)
        depth = arrays.get("depth_to_surface_m")
        burial = arrays.get("burial_depth_m")
        add(checks, "depth_to_surface_positive", depth is not None and bool(np.all(np.asarray(depth, dtype=float) > 0)), "" if depth is None else np.asarray(depth).tolist(), ">0")
        add(checks, "burial_depth_positive", burial is not None and bool(np.all(np.asarray(burial, dtype=float) > 0)), "" if burial is None else np.asarray(burial).tolist(), ">0")
    ...
    status = status_from_success(success_count, len(plan_rows), arrays)
    passed_checks = not [row for row in checks if row["check_name"] not in {"success_count_minimum"} and not row["pass"]]
    if success_count < 6:
        status = "blocked"
    ...
    manifest = {
        ...
        "generator_script": r"C:\Users\19166\Desktop\COMSOL_Multiphysics_MCP\scripts\generate_mfl_internal_defect_smoke_pack.py",
    ...
    summary_lines = [
        "21.0 内部/埋藏缺陷 smoke pack validation summary",
        "",
        f"dataset_id: {DATASET_ID}",
        f"status: {status}",
        f"planned_samples: {len(plan_rows)}",
        f"successful_samples: {success_count}",
        f"validation_passed_for_available_rows: {str(bool(passed_checks)).lower()}",
    ...
    summary_lines.extend(f"- {row['check_name']}: observed={row['observed']} expected={row['expected']}" for row in failed)
    if not failed:
    ...
    reg_lines = [
        "21.0 内部缺陷 smoke pack registry summary",
        "",
    ...
    if status == "blocked":
        raise RuntimeError(f"internal defect smoke pack blocked: {success_count}/{len(plan_rows)} successes")
    return 0

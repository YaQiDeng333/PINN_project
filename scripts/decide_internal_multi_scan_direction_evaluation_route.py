#!/usr/bin/env python
"""23.3 route decision for internal multi-scan-direction evaluation."""
...
from audit_internal_multi_scan_direction_pairs import ROOT, selection_score


PROBE_METRICS = ROOT / "results/metrics/internal_multi_scan_direction_diagnostic_probe_metrics.csv"
SEPARABILITY = ROOT / "results/metrics/internal_multi_scan_direction_feature_separability_metrics.csv"
PAIR_METRICS = ROOT / "results/metrics/internal_multi_scan_direction_pair_metrics.csv"
SUMMARY = ROOT / "results/summaries/internal_multi_scan_direction_evaluation_route_decision_summary.txt"
MATRIX = ROOT / "results/metrics/internal_multi_scan_direction_evaluation_decision_matrix.csv"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Decide route after 23.3 multi-scan-direction evaluation.")
...
def compare(a: dict[str, str], b: dict[str, str]) -> dict[str, float]:
    """Return b-a deltas; negative is improvement for error metrics."""
    return {
...
def mixed_signal(deltas: list[float], epsilon: float = 1e-9) -> str:
    """Classify signed deltas where negative means improvement."""
...
def run(args: argparse.Namespace) -> int:
    ...
    required = ["single_x_5line", "dual_xy_5line", "single_x_9line", "dual_xy_9line"]
    missing = [name for name in required if name not in probe]
    if missing:
        raise RuntimeError(f"missing selected test probe rows: {missing}")
    c5 = compare(probe["single_x_5line"], probe["dual_xy_5line"])
    c9 = compare(probe["single_x_9line"], probe["dual_xy_9line"])
    d9_vs_d5 = compare(probe["dual_xy_5line"], probe["dual_xy_9line"])
    sep5 = {
        "shape_delta": f(sep_rows["dual_xy_5line"], "shape_nn_consistency") - f(sep_rows["single_x_5line"], "shape_nn_consistency"),
        "ambiguous_delta": f(sep_rows["dual_xy_5line"], "ambiguous_neighbor_rate") - f(sep_rows["single_x_5line"], "ambiguous_neighbor_rate"),
        "cuboid_ellipsoid_delta": f(sep_rows["dual_xy_5line"], "cuboid_ellipsoid_cross_nn_rate") - f(sep_rows["single_x_5line"], "cuboid_ellipsoid_cross_nn_rate"),
    }
    sep9 = {
        "shape_delta": f(sep_rows["dual_xy_9line"], "shape_nn_consistency") - f(sep_rows["single_x_9line"], "shape_nn_consistency"),
        "ambiguous_delta": f(sep_rows["dual_xy_9line"], "ambiguous_neighbor_rate") - f(sep_rows["single_x_9line"], "ambiguous_neighbor_rate"),
        "cuboid_ellipsoid_delta": f(sep_rows["dual_xy_9line"], "cuboid_ellipsoid_cross_nn_rate") - f(sep_rows["single_x_9line"], "cuboid_ellipsoid_cross_nn_rate"),
    }
    ...
    optional = [row for name, row in probe.items() if name.endswith("plus_direction_features")]
    ...
    if optional_best and score(optional_best) + 0.02 < score(best_main):
        next_step = "C_train_dual_xy_plus_richer_feature_fusion_model"
        next_note = "direction-difference features outperformed plain single/dual configs in diagnostic probe"
    elif dual9_better and (nine_better or score(probe["dual_xy_9line"]) <= score(probe["dual_xy_5line"])):
        next_step = "B_train_dual_xy_9line_internal_model"
        next_note = "dual_xy_9line has the strongest diagnostic evidence"
    elif dual5_better:
        next_step = "A_train_dual_xy_5line_internal_model"
        next_note = "dual_xy_5line improves over x_only_5line with lower acquisition cost than 9line"
    elif not (dual5_better or dual9_better):
        next_step = "D_generate_multi_magnetization_diagnostic_pack"
        next_note = "dual direction did not clearly beat x-only; richer source/magnetization information is the next observation axis"
    else:
        next_step = "F_keep_abstention_only_route_and_pause_internal_refinement"
        next_note = "evidence is mixed and does not justify training"

    rows = [
        {
            "question": "dual_xy_5line 是否优于 x_only_5line",
            ...
            "decision": "支持 5line dual" if dual5_better else "5line dual 不足以单独进入训练",
        },
        {
            "question": "dual_xy_9line 是否优于 x_only_9line",
            ...
            "decision": "支持 9line dual" if dual9_better else "9line dual 不足以单独进入训练",
        },
        {
            "question": "9line 是否显著优于 5line",
            "answer": yes_no(nine_better),
            "evidence": f"dual_9_vs_dual_5_score_delta={d9_vs_d5['score_delta']:.6f}; center_p95_delta_mm={d9_vs_d5['center_p95_delta_mm']:.3f}",
            "decision": "9line 成本可能值得" if nine_better else "9line 成本优势未被证明",
        },
        {
            "question": "dual-direction 是否降低 cuboid/ellipsoid confusion",
            ...
            "decision": "混合信号：9line NN 改善，但 probe 与 5line NN 未改善" if confusion_signal == "mixed" else ("几何分支信息有改善" if confusion_signal == "yes" else "cuboid/ellipsoid 分支仍不充分"),
        },
        {
            "question": "dual-direction 是否降低 center/burial tail",
            "answer": tail_signal,
            ...
            "decision": "混合信号：5line center 改善但 burial 变差，9line burial 改善但 center 变差" if tail_signal == "mixed" else ("tail 有改善信号" if tail_signal == "yes" else "tail 没有改善信号"),
        },
        {
            "question": "是否进入 23.4 training gate",
    ...
    lines = [
        "23.3 internal multi-scan-direction evaluation route decision summary",
        "",
        f"dual_xy_5line_better_than_x_only_5line: {str(dual5_better).lower()}",
        f"dual_xy_9line_better_than_x_only_9line: {str(dual9_better).lower()}",
        f"dual_xy_9line_better_than_dual_xy_5line: {str(nine_better).lower()}",
        ...
        "formal_training_in_23_3: false",
        "",
        "结论：23.3 的 probe 只用于路线判断；不得把本轮结果写成正式模型候选或 baseline。",
    ]

...
CHECKPOINT_DIR = ROOT / 'checkpoints' / 'mask_boundary_grid_candidate'
METRICS_PATH = ROOT / 'results' / 'metrics' / 'v3_complex_mask_boundary_grid_candidate_metrics.csv'
SUMMARY_PATH = ROOT / 'results' / 'summaries' / 'v3_complex_mask_boundary_grid_candidate_summary.txt'
PREVIEW_DIR = ROOT / 'results' / 'previews' / 'mask_boundary_grid_candidate'
...
def train_one_seed(seed, device, pos_weight_value):
    ...
    best_info = None
    best_path = CHECKPOINT_DIR / f'best_mask_boundary_grid_seed{seed}.pt'
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if selection_score > best_score:
            ...
            torch.save({
                ...
                'args': {
                    'model': 'mask_boundary_grid_model',
                    'dataset': 'v3_complex',
                    'seed': seed,
                    'epochs': EPOCHS,
                    'batch_size': BATCH_SIZE,
...
def format_comparison_table(rows, group_type, groups, selected_threshold):
    ...
    for group in groups:
        ...
        grid_mean = find_row(rows, 'mask_boundary_grid_test_mean', group_type, group, threshold=selected_threshold)
        grid_std = find_row(rows, 'mask_boundary_grid_test_std', group_type, group, threshold=selected_threshold)
        lines.append(
        ...
        lines.append(
            f"| {group} | mask-only grid decoder | {selected_threshold:.2f} | "
...
def improvement_status(rows, group_type, group, selected_threshold):
    ...
    candidate = find_row(rows, 'mask_boundary_grid_test_mean', group_type, group, threshold=selected_threshold)
    return {
...
def write_summary(rows, best_infos, checkpoint_paths, selected_threshold, pos_weight, mask_fraction):
    ...
    summary = f"""# v3_complex mask-only grid decoder candidate
...
def select_preview_samples(grid_means, baseline_means):
    ...
    take('mask_boundary_grid_failure', failures, 3)
...
def main():
    ...
    best_infos = []
    for seed in SEEDS:
        print(f'Training mask boundary grid candidate seed={seed}')
    ...
    val_rows, _, _ = evaluate_checkpoint_family(
        ...
        model_type='grid',
        candidate='mask_boundary_grid_val_scan',
        split='val',
        data_path=VAL_DATA,
    ...
    test_rows, grid_sample_rows, grid_prob_cache = evaluate_checkpoint_family(
        checkpoints=grid_checkpoints,
        model_type='grid',
        candidate='mask_boundary_grid_test',
        split='test',
        data_path=TEST_DATA,

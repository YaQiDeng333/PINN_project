...
DEFAULT_NPZ = (
    ...
    / "comsol_mfl"
    / "prepared"
    / "comsol_single_defect_multiline_forward_pack_v1_pilot_v7_balanced_three_types.npz"
)
DEFAULT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_balanced_pilot_v7_training_gate_summary.txt"
DEFAULT_METRICS = PROJECT_ROOT / "results/metrics/comsol_balanced_pilot_v7_training_gate_metrics.csv"
DEFAULT_EPOCH_LOG = PROJECT_ROOT / "results/metrics/comsol_balanced_pilot_v7_training_gate_epoch_log.csv"
DEFAULT_DEFECT_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_balanced_pilot_v7_defect_type_summary.csv"
DEFAULT_ANGLE_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_balanced_pilot_v7_angle_summary.csv"
DEFAULT_VERTEX_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_balanced_pilot_v7_vertex_count_summary.csv"
DEFAULT_SOURCE_PACK_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_balanced_pilot_v7_source_pack_summary.csv"
DEFAULT_PREVIEW_DIR = PROJECT_ROOT / "results/previews/comsol_balanced_pilot_v7_gate"

THRESHOLD_CANDIDATES = [0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90]
EXPECTED_SPLITS = {"train": 162, "val": 39, "test": 39}
EXPECTED_DEFECTS = {"rectangular_notch": 80, "rotated_rect": 80, "polygon": 80}
EXPECTED_ROT_ANGLES = {-30.0, -20.0, -10.0, 10.0, 20.0, 30.0}
EXPECTED_VERTEX_COUNTS = {4: 27, 5: 27, 6: 26}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a balanced three-type COMSOL pilot_v7 training gate.")
...
def validate_balanced_v7_npz(npz_path: Path) -> dict[str, Any]:
    required = [
        "delta_bz",
        "bz_defect",
        "bz_no_defect",
            ...
            rows.append(
                {
                    ...
                    **metrics,
                    ...
                    "notes": "balanced_pilot_v7_training_gate_only",
                }
            )
    return rows, probs
...
def build_summary(context: dict[str, Any]) -> str:
    lines = [
        "# Stage 20.24 COMSOL balanced pilot_v7 training gate",
        "",
        "## 1. NPZ / schema checks",
        "",
        f"- balanced pilot_v7 NPZ readable: {context['npz_readable']}",
        f"- schema complete: {context['schema_complete']}",
        f"- split is 162 / 39 / 39: {context['split_is_162_39_39']} ({context['split_counts']})",
        ...
        f"- can fit 162 train samples: {context['can_fit_train_samples']}",
        "",
        "## 4. Pilot metrics",
        "",
        ...
        "## 6. Conclusion",
        "",
        "- This result validates the balanced mixed single-defect-type pilot_v7 read -> dataset loader -> train-only normalization -> training -> validation checkpoint/threshold selection -> test smoke evaluation -> preview chain.",
        "- It is not a v3_complex formal model result, not a candidate, and does not update CURRENT_BASELINE.",
        "- The 240-sample balanced pack is usable for next-stage data expansion, but remains pilot-level, controlled synthetic, and single-defect only.",
...
def main() -> int:
    ...
    if args.epochs < 1 or args.epochs > 200:
        raise ValueError("--epochs must be between 1 and 200 for this balanced pilot_v7 gate.")
    ...
    validation = validate_balanced_v7_npz(npz_path)
    ...
    context = {
        ...
        "split_is_162_39_39": validation["split_counts"] == EXPECTED_SPLITS,

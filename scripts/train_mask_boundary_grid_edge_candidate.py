...
CHECKPOINT_DIR = ROOT / 'checkpoints' / 'mask_boundary_grid_edge_candidate'
METRICS_PATH = ROOT / 'results' / 'metrics' / 'v3_complex_mask_boundary_grid_edge_candidate_metrics.csv'
SUMMARY_PATH = ROOT / 'results' / 'summaries' / 'v3_complex_mask_boundary_grid_edge_candidate_summary.txt'
PREVIEW_DIR = ROOT / 'results' / 'previews' / 'mask_boundary_grid_edge_candidate'
...
class MaskBoundaryGridEdgeModel(nn.Module):
    def __init__(
        self,
        signal_length,
        signal_channels=1,
    ...
    def forward(self, bz_signal, coords=None):
        ...
        if tuple(mask_logits.shape[-2:]) != self.out_shape:
            mask_logits = F.interpolate(mask_logits, size=self.out_shape, mode='bilinear', align_corners=False)
            boundary_logits = F.interpolate(boundary_logits, size=self.out_shape, mode='bilinear', align_corners=False)
        return (
            mask_logits[:, 0].reshape(batch_size, -1),
            boundary_logits[:, 0].reshape(batch_size, -1),
        )
...
def boundary_band_from_mask(mask_grid, width=BOUNDARY_BAND_PIXELS):
    mask_4d = mask_grid.unsqueeze(1)
    kernel_size = 2 * width + 1
    dilated = F.max_pool2d(mask_4d, kernel_size=kernel_size, stride=1, padding=width)
    eroded = 1.0 - F.max_pool2d(1.0 - mask_4d, kernel_size=kernel_size, stride=1, padding=width)
    return (dilated - eroded > 0.0).to(dtype=mask_grid.dtype).squeeze(1)


def compute_boundary_pos_weight(dataset):
    masks = torch.from_numpy((dataset.mu_maps < MASK_THRESHOLD_NORM).astype(np.float32))
    pos = 0.0
    total = 0.0
    for start in range(0, masks.shape[0], 64):
        boundary = boundary_band_from_mask(masks[start:start + 64])
        pos += float(boundary.sum().item())
        total += float(boundary.numel())
    neg = total - pos
    raw = np.sqrt(neg / max(pos, 1.0))
    return float(min(raw, POS_WEIGHT_CAP)), float(pos / total)
...
def edge_loss(mask_logits, boundary_logits, target_mask, target_boundary, mask_pos_weight, boundary_pos_weight):
    mask_total, mask_bce, mask_dice = mask_loss(mask_logits, target_mask, mask_pos_weight)
    boundary_bce = F.binary_cross_entropy_with_logits(
        boundary_logits,
        target_boundary,
        pos_weight=boundary_pos_weight,
    )
    return mask_total + LAMBDA_BOUNDARY * boundary_bce, mask_bce, mask_dice, boundary_bce
...
def load_edge_checkpoint(path, signal_length, signal_channels, out_shape, device):
    checkpoint = torch.load(path, map_location=device)
    args = checkpoint.get('args', {})
    model = MaskBoundaryGridEdgeModel(
        signal_length=signal_length,
        signal_channels=int(args.get('signal_channels', signal_channels)),
        latent_dim=int(args.get('latent_dim', LATENT_DIM)),
        out_shape=tuple(args.get('out_shape', out_shape)),
...
def train_one_seed(seed, device, pos_weight_value, boundary_pos_weight_value):
    set_seed(seed)
    ...
    boundary_pos_weight = torch.tensor(boundary_pos_weight_value, dtype=torch.float32, device=device)

    best_score = -float('inf')
    best_info = None
    best_path = CHECKPOINT_DIR / f'best_mask_boundary_grid_edge_seed{seed}.pt'
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for signals, mu_targets, indices in train_loader:
            ...
            target_boundary = boundary_band_from_mask(
                target_mask.reshape(target_mask.shape[0], *out_shape),
                BOUNDARY_BAND_PIXELS,
            ).reshape_as(target_mask)
            optimizer.zero_grad(set_to_none=True)
            mask_logits, boundary_logits = model(signals, coords)
            loss, bce, dice, boundary_bce = edge_loss(
                mask_logits,
                boundary_logits,
                target_mask,
                target_boundary,
            ...
            total_boundary += float(boundary_bce.item()) * batch_size
        ...
        if selection_score > best_score:
            ...
            torch.save({
                ...
                'args': {
                    'model': 'mask_boundary_grid_edge_model',
                    'dataset': 'v3_complex',
                    'seed': seed,
                    'epochs': EPOCHS,
                    'batch_size': BATCH_SIZE,
                    'latent_dim': LATENT_DIM,
                    'loss': 'BCEWithLogits + soft Dice + 0.1 * boundary BCEWithLogits',
                    'pos_weight': pos_weight_value,
                    'boundary_pos_weight': boundary_pos_weight_value,
                    'mask_target': 'target_mu_norm < 0.5',
                    'boundary_target': '1-pixel morphological boundary band from target mask',
                    'lambda_boundary': LAMBDA_BOUNDARY,
                    'boundary_band_pixels': BOUNDARY_BAND_PIXELS,
                    'decoder': 'latent MLP projection to low-resolution feature map plus ConvTranspose2d upsampling',
                    'out_shape': out_shape,
                    'low_shape': GRID_LOW_SHAPE,
                    'base_channels': GRID_BASE_CHANNELS,
        ...
        print(
            ...
            f"boundary_bce={total_boundary / total_samples:.6e} | "
...
def evaluate_checkpoint_family(checkpoints, model_type, candidate, split, data_path, thresholds, device, area_edges, low_signal_indices):
    ...
    for seed, checkpoint_path in checkpoints.items():
        ...
        if model_type == 'edge':
            model, _ = load_edge_checkpoint(Path(project_path(checkpoint_path)), signal_length, signal_channels, out_shape, device)
...
def select_threshold(rows, baseline_overall):
    validation_means = [
        row for row in rows
        if row['candidate'] == 'mask_boundary_grid_edge_val_scan_mean'
        and row['split'] == 'val'
        and row['group_type'] == 'overall'
        and row['group'] == 'all'
    ]
...
def format_comparison_table(rows, group_type, groups, selected_threshold):
    ...
    for group in groups:
        ...
        edge_mean = find_row(rows, 'mask_boundary_grid_edge_test_mean', group_type, group, threshold=selected_threshold)
        edge_std = find_row(rows, 'mask_boundary_grid_edge_test_std', group_type, group, threshold=selected_threshold)
        lines.append(
        ...
        lines.append(
            f"| {group} | grid + boundary head | {selected_threshold:.2f} | "
            f"{metric_with_std(edge_mean, edge_std, 'iou')} | "
            f"{metric_with_std(edge_mean, edge_std, 'dice')} | "
            f"{metric_with_std(edge_mean, edge_std, 'area_error')} | "
            f"{metric_with_std(edge_mean, edge_std, 'center_error')} | "
            f"{metric_with_std(edge_mean, edge_std, 'pred_area_zero')} | "
            f"{metric_with_std(edge_mean, edge_std, 'pred_area_lt_true')} | "
            f"{metric_with_std(edge_mean, edge_std, 'pred_area_gt_true')} |"
        )
    return '\n'.join(lines)
...
def improvement_status(rows, group_type, group, selected_threshold):
    ...
    candidate = find_row(rows, 'mask_boundary_grid_edge_test_mean', group_type, group, threshold=selected_threshold)
    return {
...
def write_summary(
    ...
    summary = f"""# v3_complex mask-only grid decoder + boundary head candidate
...
* boundary_logit head for a {BOUNDARY_BAND_PIXELS}-pixel morphological boundary band

The boundary target is generated from `target_mu_norm < 0.5` masks with a torch max-pool dilation/erosion boundary band. Training loss is mask BCEWithLogits + soft Dice + {LAMBDA_BOUNDARY:.2f} * boundary BCEWithLogits. No SDF, loss-weight sweep, adaptive threshold, post-processing, or ensemble is used.
...
Train boundary positive fraction: {boundary_fraction:.6f}. Boundary BCE pos_weight uses sqrt(neg/pos) capped at {POS_WEIGHT_CAP:.1f}; value used: {boundary_pos_weight:.6f}.
...
def select_preview_samples(grid_means, baseline_means):
    ...
    take('mask_boundary_grid_edge_failure', failures, 3)
...
def main():
    ...
    boundary_pos_weight, boundary_fraction = compute_boundary_pos_weight(train_dataset_for_weight)
    print(f'mask positive fraction: {mask_fraction:.6f}')
    print(f'pos_weight: {pos_weight:.6f}')
    print(f'boundary positive fraction: {boundary_fraction:.6f}')
    print(f'boundary_pos_weight: {boundary_pos_weight:.6f}')

    checkpoint_paths = []
    best_infos = []
    for seed in SEEDS:
        print(f'Training mask boundary grid + boundary head candidate seed={seed}')
        checkpoint_path, best_info = train_one_seed(seed, device, pos_weight, boundary_pos_weight)
    ...
    val_rows, _, _ = evaluate_checkpoint_family(
        ...
        model_type='edge',
        candidate='mask_boundary_grid_edge_val_scan',
        split='val',
        data_path=VAL_DATA,
    ...
    test_rows, grid_sample_rows, grid_prob_cache = evaluate_checkpoint_family(
        checkpoints=grid_checkpoints,
        model_type='edge',
        candidate='mask_boundary_grid_edge_test',
        split='test',
        data_path=TEST_DATA,

#!/usr/bin/env python
"""Prepare the balanced COMSOL mixed pilot_v9 pack.

This script combines existing rectangular_notch / rotated_rect / polygon COMSOL
pilot packs with the real pilot_v7 / pilot_v8 / pilot_v9 top-up samples. It does
not run COMSOL and does not train a model.
"""
...
DEFAULT_V9_TOPUP_DIR = (
    PROJECT_ROOT
    / "data/comsol_mfl/generated/comsol_single_defect_multiline_forward_pack_v1_pilot_v9_balanced_single_defect"
)
DEFAULT_OUTPUT_NPZ = (
    PROJECT_ROOT
    / "data/comsol_mfl/prepared/comsol_single_defect_multiline_forward_pack_v1_pilot_v9_balanced_single_defect.npz"
)
DEFAULT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_mixed_pilot_v9_balanced_pack_summary.txt"
DEFAULT_INVENTORY = PROJECT_ROOT / "results/metrics/comsol_mixed_pilot_v9_balanced_inventory.csv"
...
V9_TOPUP_MANIFEST_NAME = "manifest_comsol_single_defect_multiline_forward_pack_v1_pilot_v9_topup.csv"

TARGET_PER_TYPE = 200
SPLIT_TARGET_PER_TYPE = {"train": 134, "val": 33, "test": 33}
TARGET_TOTAL_SPLITS = {"train": 402, "val": 99, "test": 99}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare balanced COMSOL mixed pilot_v9 pack.")
    ...
    parser.add_argument("--v9-topup-dir", type=Path, default=DEFAULT_V9_TOPUP_DIR)
...
def load_topup_samples(topup_dir: Path, manifest_name: str) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if defect_type not in {"rectangular_notch", "rotated_rect", "polygon"}:
...
def split_samples_source_aware(
    samples: list[dict[str, Any]],
    ...
    split_by_index: dict[int, str] = {}
    remaining = {key: list(value) for key, value in groups.items()}
    ordered_keys = sorted(remaining, key=lambda value: (str(value[1]), str(value[0])))
    source_keys: dict[str, list[Any]] = defaultdict(list)
    for key in ordered_keys:
        source_keys[key[0]].append(key)

    def split_count(split_name: str) -> int:
        return sum(1 for value in split_by_index.values() if value == split_name)

    def take_from_keys(keys: list[Any], split_name: str) -> int | None:
        available = [key for key in keys if remaining[key]]
        if not available:
            return None
        key = max(available, key=lambda item: len(remaining[item]))
        return remaining[key].pop(0 if split_name == "val" else -1)

    for split_name in ["val", "test"]:
        target = SPLIT_TARGET_PER_TYPE[split_name]
        for source in sorted(source_keys):
            if split_count(split_name) >= target:
                break
            index = take_from_keys(source_keys[source], split_name)
            if index is not None and index not in split_by_index:
                split_by_index[index] = split_name
        cursor = 0
        while split_count(split_name) < target:
            key = ordered_keys[cursor % len(ordered_keys)]
            cursor += 1
            if not remaining[key]:
                continue
    ...
    for split_name in ["train", "val", "test"]:
        ...
        if len(split_sources) < min_source_count:
...
def inventory_row(sample: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "notes": "balanced pilot_v9 single-defect mixed sample",
    }
...
def validate_merged(samples: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if dict(defect_counts) != {"rectangular_notch": 200, "rotated_rect": 200, "polygon": 200}:
    ...
    if delta_bz.shape != (600, 3, 201) or masks.shape != (600, 64, 128):
    ...
    for split in ["train", "val", "test"]:
        ...
        for defect_type in ["rectangular_notch", "rotated_rect", "polygon"]:
            if len(split_type_source_counts[split][defect_type]) < 2:
...
def main() -> int:
    ...
    v9_topup_dir = resolve(args.v9_topup_dir)
    ...
    v9_topup_samples = load_topup_samples(v9_topup_dir, V9_TOPUP_MANIFEST_NAME)
    check_topup_coordinates(v7_topup_samples, rect_pack)
    check_topup_coordinates(v8_topup_samples, rect_pack)
    check_topup_coordinates(v9_topup_samples, rect_pack)
    ...
    v9_topup_counts = Counter(sample["defect_type"] for sample in v9_topup_samples)
    ...
    if dict(v9_topup_counts) != {"rectangular_notch": 80, "rotated_rect": 80, "polygon": 80}:
        raise PackValidationError(f"pilot_v9 top-up distribution mismatch: {dict(v9_topup_counts)}")
    topup_samples = v7_topup_samples + v8_topup_samples + v9_topup_samples

    rect_samples = build_existing_samples(rect_pack, "pilot_v2_rectangular_notch", "rectangular_notch", list(range(120))) + [
        sample for sample in v9_topup_samples if sample["defect_type"] == "rectangular_notch"
    ]
    ...
    if len(rect_samples) != 200 or len(rotated_samples) != 200 or len(polygon_samples) != 200:
        raise PackValidationError(
            f"composition mismatch: rect={len(rect_samples)}, rotated={len(rotated_samples)}, polygon={len(polygon_samples)}"
        )

    split_samples_source_aware(
        rect_samples,
        lambda item: (round(float(item["geometry"]["width"] or 0.0), 6), round(float(item["geometry"]["depth"] or 0.0), 6)),
        min_source_count=2,
    )
    split_samples_source_aware(rotated_samples, lambda item: float(item["geometry"]["angle_deg"]), min_source_count=2)
    split_samples_source_aware(polygon_samples, lambda item: int(item["geometry"]["vertex_count"]), min_source_count=2)
    samples = assign_ids(
        {
    ...
    metadata = {
        "pack_name": "comsol_single_defect_multiline_forward_pack_v1_pilot_v9_balanced_single_defect",
        "source_packs": [
            str(rect_npz),
            str(rot_npz),
            str(poly_npz),
            ...
            str(v9_topup_dir / V9_TOPUP_MANIFEST_NAME),
        ],
        ...
        "note": "balanced pilot_v9 mixed single-defect-type pack; not official full dataset and not multi_defect",
    }
    if not args.dry_run:
        ...
        ensure_source_pack("pilot_v9_balanced", merged, 600, "rectangular_notch") if False else None
        if merged["delta_bz"].shape != (600, 3, 201) or merged["masks"].shape != (600, 64, 128):
    ...
    summary_lines = [
        "# COMSOL mixed pilot_v9 balanced pack summary",
        "",
        ...
        f"pilot_v9 top-up readable: yes ({v9_topup_dir / V9_TOPUP_MANIFEST_NAME})",
        "coordinate consistency: yes (sensor_x, scan_line_y, mask_x, mask_y all match)",
        f"balanced pilot_v9 NPZ generated: {'no, dry-run only' if args.dry_run else 'yes'}",
        f"balanced pilot_v9 NPZ path: {output_npz}",
        ...
        "val/test source_pack distribution no longer single-source for rectangular_notch / rotated_rect / polygon: yes",
        ...
        "schema-ready: yes",
        "pilot_v9 train-ready: yes, for balanced pilot smoke/training gate only",
        "current limitations: still pilot-level, single-defect only, no multi_defect, limited COMSOL geometry families",
        "next step: enter balanced pilot_v9 training gate in PINN_project",

...
DEFAULT_NPZ = PROJECT_ROOT / "data/comsol_mfl/prepared/comsol_multi_defect_three_component_multiline_forward_pack_v2_pilot.npz"
DEFAULT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_three_component_multi_defect_pilot_v2_training_gate_summary.txt"
DEFAULT_AUDIT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_three_component_multi_defect_pilot_v2_failure_audit_summary.txt"
DEFAULT_PLAN = PROJECT_ROOT / "results/summaries/comsol_three_component_multi_defect_pilot_v3_expansion_plan.txt"
DEFAULT_METRICS = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v2_training_gate_metrics.csv"
DEFAULT_EPOCH_LOG = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v2_epoch_log.csv"
DEFAULT_SEED_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v2_seed_summary.csv"
DEFAULT_COMBO_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v2_component_combination_summary.csv"
DEFAULT_CONNECTED_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v2_connected_component_summary.csv"
DEFAULT_FAILURE_CASES = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v2_failure_audit_cases.csv"
DEFAULT_PREVIEW_DIR = PROJECT_ROOT / "results/previews/comsol_three_component_multi_defect_pilot_v2_gate"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run COMSOL three-component multi_defect pilot_v2 3-seed training gate.")
...
def validate_npz(npz_path: Path) -> dict[str, Any]:
    ...
    if any(combo_counts.get(combo, 0) != 30 for combo in EXPECTED_COMBOS):
    ...
    if dict(distance_counts) != {"near": 40, "medium": 40, "far": 40}:
    ...
    for combo in sorted(EXPECTED_COMBOS):
        ...
        if dict(combo_distance_counts) != {"near": 10, "medium": 10, "far": 10}:
            raise ThreeComponentValidationError(f"unexpected distance_bin counts for {combo}: {dict(combo_distance_counts)}")
        for split_name, expected_count in (("train", 20), ("val", 5), ("test", 5)):
...
def build_training_summary(context: dict[str, Any]) -> str:
    lines = [
        "# COMSOL true 3-component multi_defect pilot_v2 3-seed training gate",
        "",
        "## Schema",
        "",
...
def build_audit_summary(context: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# COMSOL true 3-component multi_defect pilot_v2 failure audit",
            "",
...
def build_expansion_plan(context: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# COMSOL 3-component multi_defect pilot_v3 expansion plan",
            "",
            "- Current pack: 120 true 3-component multi_defect pilot_v2 samples with 80 train / 20 val / 20 test.",

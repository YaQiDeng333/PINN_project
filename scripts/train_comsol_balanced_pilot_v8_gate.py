...
DEFAULT_NPZ = (
    ...
    / "comsol_mfl"
    / "prepared"
    / "comsol_single_defect_multiline_forward_pack_v1_pilot_v8_balanced_three_types.npz"
)
DEFAULT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_balanced_pilot_v8_training_gate_summary.txt"
DEFAULT_METRICS = PROJECT_ROOT / "results/metrics/comsol_balanced_pilot_v8_training_gate_metrics.csv"
DEFAULT_EPOCH_LOG = PROJECT_ROOT / "results/metrics/comsol_balanced_pilot_v8_training_gate_epoch_log.csv"
DEFAULT_DEFECT_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_balanced_pilot_v8_defect_type_summary.csv"
DEFAULT_ANGLE_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_balanced_pilot_v8_angle_summary.csv"
DEFAULT_VERTEX_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_balanced_pilot_v8_vertex_count_summary.csv"
DEFAULT_SOURCE_PACK_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_balanced_pilot_v8_source_pack_summary.csv"
DEFAULT_PREVIEW_DIR = PROJECT_ROOT / "results/previews/comsol_balanced_pilot_v8_gate"

THRESHOLD_CANDIDATES = [0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90]
EXPECTED_SPLITS = {"train": 240, "val": 60, "test": 60}
EXPECTED_DEFECTS = {"rectangular_notch": 120, "rotated_rect": 120, "polygon": 120}
EXPECTED_ROT_ANGLES = {-30.0, -20.0, -10.0, 10.0, 20.0, 30.0}
EXPECTED_ROT_ANGLE_COUNTS = {-30.0: 20, -20.0: 20, -10.0: 20, 10.0: 20, 20.0: 20, 30.0: 20}
EXPECTED_VERTEX_COUNTS = {4: 40, 5: 40, 6: 40}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a balanced three-type COMSOL pilot_v8 training gate.")
...
def validate_balanced_v8_npz(npz_path: Path) -> dict[str, Any]:
    required = [
        "delta_bz",
        "bz_defect",
        "bz_no_defect",
    ...
    if rotated_angle_distribution != EXPECTED_ROT_ANGLE_COUNTS:
        raise RuntimeError(f"unexpected rotated angle distribution: {rotated_angle_distribution}")
    ...
    for split_name in ("train", "val", "test"):
        ...
        for defect_name in ("rotated_rect", "polygon"):
            if len(split_type_source_counts[split_name][defect_name]) < 3:
                raise RuntimeError(
    ...
    return {
        ...
        "rotated_angle_distribution": rotated_angle_distribution,
            ...
            rows.append(
                {
                    ...
                    **metrics,
                    ...
                    "notes": "balanced_pilot_v8_training_gate_only",
                }
            )
    return rows, probs
...
def build_summary(context: dict[str, Any]) -> str:
    lines = [
        "# Stage 20.26 COMSOL balanced pilot_v8 training gate",
        "",
        "## 1. NPZ / schema checks",
        "",
        f"- balanced pilot_v8 NPZ readable: {context['npz_readable']}",
        f"- schema complete: {context['schema_complete']}",
        f"- split is 240 / 60 / 60: {context['split_is_240_60_60']} ({context['split_counts']})",
        ...
        f"- source_pack distribution: {context['source_pack_distribution']}",
        f"- split source_pack distribution for each defect_type: {context['split_type_source_counts']}",
        f"- val/test source_pack not single-source for rotated_rect and polygon: {context['val_test_source_pack_not_single']}",
        ...
        f"- can fit 240 train samples: {context['can_fit_train_samples']}",
        "",
        "## 4. Pilot metrics",
        "",
        ...
        f"- source_pack summary: {context['source_pack_summary_path']}",
        ...
        "## 6. Conclusion",
        "",
        "- This result validates the balanced mixed single-defect-type pilot_v8 read -> dataset loader -> train-only normalization -> training -> validation checkpoint/threshold selection -> test evaluation -> preview chain.",
        "- It is not a v3_complex formal model result, not a candidate, and does not update CURRENT_BASELINE.",
        "- The 360-sample balanced pack is usable for next-stage data expansion, but remains pilot-level, controlled synthetic, and single-defect only.",
...
def main() -> int:
    ...
    if args.epochs < 1 or args.epochs > 200:
        raise ValueError("--epochs must be between 1 and 200 for this balanced pilot_v8 gate.")
    ...
    validation = validate_balanced_v8_npz(npz_path)
    ...
    source_pack_rows = group_summary(metric_rows, "source_pack", source_pack_values)
    ...
    context = {
        ...
        "split_is_240_60_60": validation["split_counts"] == EXPECTED_SPLITS,
        ...
        "source_pack_distribution": validation["source_pack_distribution"],
        "split_type_source_counts": validation["split_type_source_counts"],
        "val_test_source_pack_not_single": all(
            len(validation["split_type_source_counts"][split_name][defect_name]) >= 3

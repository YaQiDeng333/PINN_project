...
DEFAULT_NPZ = PROJECT_ROOT / "data/comsol_mfl/prepared/comsol_multi_defect_three_component_multiline_forward_pack_v4_pilot.npz"
DEFAULT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_three_component_multi_defect_pilot_v4_training_gate_summary.txt"
DEFAULT_AUDIT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_three_component_multi_defect_pilot_v4_failure_audit_summary.txt"
DEFAULT_PLAN = PROJECT_ROOT / "results/summaries/comsol_three_component_multi_defect_baseline_readiness_plan.txt"
DEFAULT_METRICS = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v4_training_gate_metrics.csv"
DEFAULT_EPOCH_LOG = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v4_epoch_log.csv"
DEFAULT_SEED_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v4_seed_summary.csv"
DEFAULT_COMBO_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v4_component_combination_summary.csv"
DEFAULT_CONNECTED_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v4_connected_component_summary.csv"
DEFAULT_FAILURE_CASES = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_v4_failure_audit_cases.csv"
DEFAULT_PREVIEW_DIR = PROJECT_ROOT / "results/previews/comsol_three_component_multi_defect_pilot_v4_gate"

SEEDS = [42, 123, 2026]
EXPECTED_SPLITS = {"train": 320, "val": 80, "test": 80}
EXPECTED_COMBOS = {
...
FAILURE_FIELDS = pilot_v1.FAILURE_FIELDS + [
    "seed",
    "extra_fragment_flag",
    "third_largest_component_area_ratio",
    "distance_bin",
...
_BASE_EXTENDED_SAMPLE_METRICS = pilot_v1.extended_sample_metrics


class ThreeComponentValidationError(RuntimeError):
    pass


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run COMSOL three-component multi_defect pilot_v4 3-seed training gate.")
...
def rasterize_component(component: dict[str, Any], mask_x: np.ndarray, mask_y: np.ndarray) -> np.ndarray:
    if tiny.as_text(component.get("component_type")) != "polygon":
...
def validate_npz(npz_path: Path) -> dict[str, Any]:
    ...
    if missing:
        raise ThreeComponentValidationError(f"missing NPZ fields: {missing}")
    ...
    source_pack = np.array([tiny.as_text(item) for item in data["source_pack"].tolist()])
    ...
    if delta_bz.shape != (480, 3, 201):
        raise ThreeComponentValidationError(f"unexpected delta_bz shape: {delta_bz.shape}")
    if bz_defect.shape != delta_bz.shape or bz_no_defect.shape != delta_bz.shape:
        raise ThreeComponentValidationError("bz_defect / bz_no_defect shape mismatch")
    if masks.shape != (480, 64, 128):
        raise ThreeComponentValidationError(f"unexpected masks shape: {masks.shape}")
    if sensor_x.shape != (201,) or scan_line_y.shape != (3,) or mask_x.shape != (128,) or mask_y.shape != (64,):
        raise ThreeComponentValidationError("coordinate shape mismatch")
    if not (
        ...
        raise ThreeComponentValidationError("coordinates are not strictly increasing")
    if not all(np.isfinite(array).all() for array in (delta_bz, bz_defect, bz_no_defect, masks, sensor_x, scan_line_y, mask_x, mask_y)):
        raise ThreeComponentValidationError("non-finite arrays found")
    if not np.allclose(delta_bz, bz_defect - bz_no_defect, rtol=1e-9, atol=1e-12):
        raise ThreeComponentValidationError("delta_bz mismatch")
    if float(delta_bz.std()) <= 0.0:
        raise ThreeComponentValidationError("delta_bz is zero")
    if np.max(np.abs(delta_bz[:, 1:, :] - delta_bz[:, :1, :])) <= 1e-12:
        raise ThreeComponentValidationError("scan lines are identical")
    if not np.all(masks.reshape(masks.shape[0], -1).sum(axis=1) > 0):
        raise ThreeComponentValidationError("empty mask found")
    if set(defect_types.tolist()) != {"multi_defect"}:
        raise ThreeComponentValidationError(f"unexpected defect_types: {Counter(defect_types.tolist())}")
    if not np.all(component_counts == 3):
        raise ThreeComponentValidationError(f"component_counts not all 3: {Counter(component_counts.tolist())}")
    if not np.all(connected_counts == 3):
        raise ThreeComponentValidationError(f"connected_component_counts not all 3: {Counter(connected_counts.tolist())}")
    if len(set(sample_ids.tolist())) != len(sample_ids):
        raise ThreeComponentValidationError("sample_ids are not unique")
    split_counts = Counter(splits.tolist())
    if dict(split_counts) != EXPECTED_SPLITS:
        raise ThreeComponentValidationError(f"unexpected split counts: {dict(split_counts)}")
    source_counts = Counter(source_pack.tolist())
    if dict(source_counts) != {"pilot_v3": 240, "pilot_v4_topup": 240}:
        raise ThreeComponentValidationError(f"unexpected source_pack counts: {dict(source_counts)}")
    ...
    for index, component_list in enumerate(components):
        if not isinstance(component_list, list) or len(component_list) != 3:
            raise ThreeComponentValidationError(f"components_json index {index} is not a 3-component list")
        ...
        if combo not in EXPECTED_COMBOS:
            raise ThreeComponentValidationError(f"unexpected component combo: {combo}")
        ...
        for component in component_list:
            ...
            if int(component_mask.sum()) <= 0:
                raise ThreeComponentValidationError(f"empty component mask at sample {index}")
        ...
        if cc != 3:
            raise ThreeComponentValidationError(f"mask connected component count at {index} is {cc}")
        pairwise = [
            float(math.hypot(centers[i][0] - centers[j][0], centers[i][1] - centers[j][1]))
            for i in range(len(centers))
            for j in range(i + 1, len(centers))
        ]
        min_distances.append(float(min(pairwise)))
    if min(geometry_mask_ious) < 0.999:
        raise ThreeComponentValidationError(f"components_json does not explain masks: min IoU {min(geometry_mask_ious)}")

    areas = masks.reshape(masks.shape[0], -1).sum(axis=1).astype(np.float64)
    combo_counts = Counter(component_type_combos)
    expected_combo_count = delta_bz.shape[0] // len(EXPECTED_COMBOS)
    if any(combo_counts.get(combo, 0) != expected_combo_count for combo in EXPECTED_COMBOS):
        raise ThreeComponentValidationError(f"unexpected component combo counts: {dict(combo_counts)}")
    distance_bins = np.array([tiny.as_text(item) for item in data["distance_bins"].tolist()])
    if set(distance_bins.tolist()) != {"near", "medium", "far"}:
        raise ThreeComponentValidationError(f"distance_bins do not cover near/medium/far: {Counter(distance_bins.tolist())}")
    distance_counts = Counter(distance_bins.tolist())
    expected_distance_count = delta_bz.shape[0] // 3
    if dict(distance_counts) != {"near": expected_distance_count, "medium": expected_distance_count, "far": expected_distance_count}:
        raise ThreeComponentValidationError(f"unexpected distance_bin counts: {dict(distance_counts)}")
    for combo in sorted(EXPECTED_COMBOS):
        combo_mask = np.array(component_type_combos) == combo
        combo_distance_counts = Counter(distance_bins[combo_mask].tolist())
        expected_combo_distance_count = expected_combo_count // 3
        if dict(combo_distance_counts) != {
            "near": expected_combo_distance_count,
            "medium": expected_combo_distance_count,
            "far": expected_combo_distance_count,
        }:
            raise ThreeComponentValidationError(f"unexpected distance_bin counts for {combo}: {dict(combo_distance_counts)}")
        expected_split_combo_counts = (("train", 80), ("val", 20), ("test", 20))
        for split_name, expected_count in expected_split_combo_counts:
            split_combo_count = int(np.sum(combo_mask & (splits == split_name)))
            if split_combo_count != expected_count:
                raise ThreeComponentValidationError(f"{split_name}/{combo} count={split_combo_count}, expected {expected_count}")
    for split_name in ("train", "val", "test"):
        split_distance_bins = set(distance_bins[splits == split_name].tolist())
        if split_distance_bins != {"near", "medium", "far"}:
            raise ThreeComponentValidationError(f"{split_name} does not cover all distance bins: {split_distance_bins}")
        split_sources = set(source_pack[splits == split_name].tolist())
        if split_sources != {"pilot_v3", "pilot_v4_topup"}:
            raise ThreeComponentValidationError(f"{split_name} does not cover both source packs: {split_sources}")
    ...
    return {
        ...
        "source_pack_counts": dict(source_counts),
...
def summarize(rows: list[dict[str, Any]], group: str, split: str, seed: Any = "all") -> dict[str, Any]:
    ...
    base["predicted_component_count_is_3_rate"] = pilot_v1.mean_or_nan(
        [float(row["predicted_component_count_is_3"]) for row in rows]
    )
    base["extra_fragment_rate"] = pilot_v1.mean_or_nan([float(row["extra_fragment_flag"]) for row in rows])
    base.pop("predicted_component_count_is_2_rate", None)
...
def extended_sample_metrics_three(
    prob: np.ndarray,
    target: np.ndarray,
    threshold: float,
    ...
    base = _BASE_EXTENDED_SAMPLE_METRICS(prob, target, threshold, components, mask_x, mask_y)
    pred = prob >= threshold
    true = target >= 0.5
    true_cc, _, _ = pilot_v1.connected_components(true)
    pred_cc, pred_areas, _ = pilot_v1.connected_components(pred)
    sorted_areas = sorted(pred_areas, reverse=True)
    pred_area = max(1, int(pred.sum()))
    recall = pilot_v1.component_recall(pred, components, mask_x, mask_y)
    ...
    merged = int(pred_cc < true_cc and recall >= 1.0)
...
def train_one_seed(
    ...
    for row in metric_rows:
        ...
        row["distance_bin"] = tiny.as_text(validation["distance_bins"][idx])
        row["source_pack"] = tiny.as_text(validation["source_pack"][idx])
...
def failure_cases(metric_rows: list[dict[str, Any]], preview_paths: dict[str, str]) -> list[dict[str, Any]]:
    ...
    for row in sorted(seed42_rows, key=lambda item: float(item["dice"]))[:10]:
        selected[tiny.as_text(row["sample_id"])] = row
    for row in sorted(seed42_rows, key=lambda item: float(item["area_error"]), reverse=True)[:10]:
        selected[tiny.as_text(row["sample_id"])] = row
    for row in sorted(seed42_rows, key=lambda item: float(item["component_count_error"]), reverse=True)[:10]:
        selected[tiny.as_text(row["sample_id"])] = row
    for combo in sorted(EXPECTED_COMBOS):
        rows = [row for row in seed42_rows if row["component_types"] == combo]
        if rows:
            selected[tiny.as_text(min(rows, key=lambda item: float(item["dice"]))["sample_id"])] = min(
                rows, key=lambda item: float(item["dice"])
            )
    cases: list[dict[str, Any]] = []
    for row in sorted(selected.values(), key=lambda item: (item["split"], float(item["dice"]))):
        category, note = failure_category_three(row)
        cases.append(
            {
                ...
                "seed": SEEDS[0],
                "extra_fragment_flag": row["extra_fragment_flag"],
                "third_largest_component_area_ratio": row["third_largest_component_area_ratio"],
                "distance_bin": row.get("distance_bin", row.get("min_component_distance_bin", "")),
...
def failure_category_three(row: dict[str, Any]) -> tuple[str, str]:
    if int(row["missed_component_flag"]) == 1:
        return "missed component", "component recall heuristic is below 1.0 for the three-component target."
    if int(row["merged_component_flag"]) == 1:
        return "component merge", "prediction has fewer connected components than the true three-component union."
    if int(row["split_component_flag"]) == 1:
        return "extra fragments", "prediction has more connected components than the true three-component union mask."
    ...
    if not math.isnan(float(row["center_error"])) and float(row["center_error"]) > 5.0:
        return "localization", "predicted mask center is far from the true union mask center."
    return "boundary / shape smoothing", "prediction overlaps the union mask but boundaries are smoothed or coarse."


def build_training_summary(context: dict[str, Any]) -> str:
    lines = [
        "# COMSOL true 3-component multi_defect pilot_v4 3-seed training gate",
        "",
        "## Schema",
        "",
        f"- three-component NPZ readable: {context['npz_readable']}",
        f"- schema complete: {context['schema_complete']}",
        f"- split is 320 / 80 / 80: {context['split_ok']} ({context['split_counts']})",
        f"- component_count all 3: {context['component_count_all_3']}",
        f"- true connected count all 3: {context['true_cc_all_3']}",
        f"- component_type combination distribution: {context['combo_counts']}",
        f"- min_component_distance bins: {context['distance_bin_counts']}",
        f"- source_pack distribution: {context['source_pack_counts']}",
        ...
        f"- can fit train samples: {context['can_fit_train_samples']}",
        "",
        ...
        f"- missed / merged / split / extra rates on test: {context['test_component_error_summary']}",
        ...
        "## Conclusion",
        "",
        "- This is a 3-component multi_defect pilot gate only, not a baseline and not a v3_complex comparison.",
        f"- recommend expanding component_count>2 data: {context['recommend_expand']}",
        f"- recommend component_count>2 COMSOL baseline now: {context['recommend_baseline']}",
...
def build_audit_summary(context: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# COMSOL true 3-component multi_defect pilot_v4 failure audit",
            "",
            f"- model predicts non-empty union masks: {not context['has_empty_prediction']}",
            f"- missed component rate on test: {context['test_missed_rate']:.4f}",
            f"- merged component rate on test: {context['test_merged_rate']:.4f}",
            f"- split component rate on test: {context['test_split_rate']:.4f}",
            f"- extra fragment rate on test: {context['test_extra_rate']:.4f}",
...
def build_expansion_plan(context: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# COMSOL 3-component multi_defect baseline readiness plan",
            "",
            "- Current pack: true 3-component multi_defect pilot_v4 samples with 320 train / 80 val / 80 test for the full 480-sample run.",
            "- Component combinations include rectangular_notch+rectangular_notch+rectangular_notch, rectangular_notch+rectangular_notch+rotated_rect, rectangular_notch+rotated_rect+rotated_rect, and rotated_rect+rotated_rect+rotated_rect.",
            "- component_count=3 is fixed; only rectangular_notch/rotated_rect components are included.",
            "- min_component_distance is stratified into near / medium / far bins and each split covers all bins.",
            "- source_pack is tracked for pilot_v3 and pilot_v4_topup; it is used only for metrics, never as model input.",
            ...
            "- Do not update existing baseline documents in this run.",
            "- Baseline-readiness condition: schema complete, no data corruption, no systematic missed third component, stable 3-seed val/test behavior, and val/test contain all combinations, distance bins, and source packs.",
            "- Stop condition: COMSOL geometry instability, components_json/mask mismatch, or frequent predicted connected count collapse below 3 on val/test.",
            f"- Current three-component pilot signal: {context['recommendation']}",
        ]
    ) + "\n"
...
def main() -> int:
    ...
    pilot_v1.extended_sample_metrics = extended_sample_metrics_three
    ...
    combo_rows = build_group_rows(all_metric_rows, "component_types", sorted(EXPECTED_COMBOS))
    component_rows = list(combo_rows)
    ...
    source_rows = build_group_rows(all_metric_rows, "source_pack", ["pilot_v3", "pilot_v4_topup"])
    component_rows.extend(distance_rows)
    component_rows.extend(area_rows)
    component_rows.extend(source_rows)
    ...
    test_extra_rate = float(np.mean([float(row["extra_fragment_flag"]) for row in test_rows]))
    test_cc3_rate = float(np.mean([float(row["predicted_component_count_is_3"]) for row in test_rows]))
    ...
    hardest = hardest_combo(combo_rows)
    ...
    test_dice_mean, test_dice_std = mean_std(seed_results, "test", "dice_mean")
    test_iou_mean, test_iou_std = mean_std(seed_results, "test", "iou_mean")
    recommend_baseline = (
        test_dice_mean >= 0.78
        and test_iou_mean >= 0.65
        and test_dice_std <= 0.02
        and test_cc3_rate >= 0.95
        and test_missed_rate <= 0.05
        and test_merged_rate <= 0.01
        and test_split_rate <= 0.08
        and not has_nan
    )
    recommend_expand = not recommend_baseline
    ...
    context = {
        ...
        "test_component_error_summary": {
            ...
            "extra_fragment_rate": test_extra_rate,
            "predicted_component_count_is_3_rate": test_cc3_rate,
        },
        ...
        "failure_main": (
            "Three-component pilot is trainable and component diagnostics are meaningful; dominant failures remain "
            f"{dominant}, with test missed/merged/split/extra rates "
            f"{test_missed_rate:.3f}/{test_merged_rate:.3f}/{test_split_rate:.3f}/{test_extra_rate:.3f}."
        ),
        "recommend_expand": recommend_expand,
        "recommend_baseline": recommend_baseline,
        "limitations": "three-component pilot only, rectangular_notch/rotated_rect components only, no polygon components, no real experimental data, not a baseline",
    }
    audit_context = {
        **context,
        ...
        "test_extra_rate": test_extra_rate,
        "dominant_failure_category": dominant,
        "near_distance_issue": near_issue,
        "interpretation": context["failure_main"],
        "recommendation": (
            "The 480-sample component_count=3 pack is stable enough to enter an external review for a component_count=3 COMSOL baseline candidate, but this script does not create that baseline."
            if recommend_baseline
            else "Do not establish a component_count>2 COMSOL baseline yet; use this pilot to guide a larger 3-component pack."
        ),
    }

...
DEFAULT_NPZ = PROJECT_ROOT / "data/comsol_mfl/prepared/comsol_multi_defect_three_component_multiline_forward_pack_v1_pilot.npz"
DEFAULT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_three_component_multi_defect_pilot_training_gate_summary.txt"
DEFAULT_AUDIT_SUMMARY = PROJECT_ROOT / "results/summaries/comsol_three_component_multi_defect_pilot_failure_audit_summary.txt"
DEFAULT_PLAN = PROJECT_ROOT / "results/summaries/comsol_three_component_multi_defect_expansion_plan.txt"
DEFAULT_METRICS = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_training_gate_metrics.csv"
DEFAULT_EPOCH_LOG = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_epoch_log.csv"
DEFAULT_SEED_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_seed_summary.csv"
DEFAULT_COMBO_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_component_combination_summary.csv"
DEFAULT_CONNECTED_SUMMARY = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_connected_component_summary.csv"
DEFAULT_FAILURE_CASES = PROJECT_ROOT / "results/metrics/comsol_three_component_multi_defect_pilot_failure_audit_cases.csv"
DEFAULT_PREVIEW_DIR = PROJECT_ROOT / "results/previews/comsol_three_component_multi_defect_pilot_gate"

SEEDS = [42, 123, 2026]
EXPECTED_SPLITS = {"train": 40, "val": 10, "test": 10}
EXPECTED_COMBOS = {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run COMSOL three-component multi_defect pilot 3-seed training gate.")
...
def build_training_summary(context: dict[str, Any]) -> str:
    lines = [
        "# COMSOL true 3-component multi_defect pilot 3-seed training gate",
        "",
        "## Schema",
        "",
        f"- three-component NPZ readable: {context['npz_readable']}",
        f"- schema complete: {context['schema_complete']}",
        f"- split is 40 / 10 / 10: {context['split_ok']} ({context['split_counts']})",
...
def build_audit_summary(context: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# COMSOL true 3-component multi_defect pilot failure audit",
            "",
...
def build_expansion_plan(context: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# COMSOL 3-component multi_defect expansion plan",
            "",
            "- Current pack: 60 true 3-component multi_defect pilot samples with 40 train / 10 val / 10 test.",
            ...
            "- Do not merge with baseline data or declare a component_count>2 baseline from this 60-sample pilot alone.",
